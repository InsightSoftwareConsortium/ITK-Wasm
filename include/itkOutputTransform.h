      ...
      if (!this->m_Transform.IsNull() && !this->m_Identifier.empty())
      {
        ...
        else
        {
          const auto fixedParamsAddress =
            reinterpret_cast<size_t>(wasmTransform->GetTransform()->GetFixedParameters().data_block());
          const auto fixedParamsSize =
            wasmTransform->GetTransform()->GetFixedParameters().Size() * sizeof(FixedParametersValueType);
          setMemoryStoreOutputArray(0, index, 0, fixedParamsAddress, fixedParamsSize);

          const auto paramsAddress =
            reinterpret_cast<size_t>(wasmTransform->GetTransform()->GetParameters().data_block());
...
template <typename TTransform>
bool
lexical_cast(const std::string & input, OutputTransform<TTransform> & outputTransform)
{
