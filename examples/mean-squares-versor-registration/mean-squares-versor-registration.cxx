...
main(int argc, char * argv[])
{
  ...
  using FixedInputImageType = itk::wasm::InputImage<FixedImageType>;
  FixedInputImageType fixedInputImage;
  pipeline.add_option("fixed-image", fixedInputImage, "Input fixed image")->required()->type_name("INPUT_IMAGE");

  using MovingInputImageType = itk::wasm::InputImage<MovingImageType>;
  MovingInputImageType movingInputImage;
  pipeline.add_option("moving-image", movingInputImage, "Input moving image")->required()->type_name("INPUT_IMAGE");

  using IOOutputImageType = itk::wasm::OutputImage<OutputImageType>;
  ...
  registration->SetFixedImage(fixedInputImage.Get());
  registration->SetMovingImage(fixedInputImage.Get());
  ...
  initializer->SetFixedImage(fixedInputImage.Get());
  initializer->SetMovingImage(movingInputImage.Get());
  // Software Guide : EndCodeSnippet
  ...
  resampler->SetInput(movingInputImage.Get());

  FixedImageType::ConstPointer fixedImage = fixedInputImage.Get();
