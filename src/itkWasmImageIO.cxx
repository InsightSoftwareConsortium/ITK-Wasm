...
bool
WasmImageIO ::SupportsDimension(unsigned long itkNotUsed(dimension))
{
  return true;
}
...
void
WasmImageIO ::PrintSelf(std::ostream & os, Indent indent) const
{
...
bool
WasmImageIO ::CanReadFile(const char * filename)
{
...
void
WasmImageIO ::SetJSON(const ImageJSON & imageJSON)
{
...
void
WasmImageIO ::ReadCBOR(void * buffer, unsigned char * cborBuffer, size_t cborBufferLength)
{
  ...
  for (size_t ii = 0; ii < indexCount; ++ii)
  {
    ...
    if (key == "imageType")
    {
      ...
      {
        const std::string_view imageTypeKey(reinterpret_cast<char *>(cbor_string_handle(imageTypeHandle[jj].key)),
                                            cbor_string_length(imageTypeHandle[jj].key));
        ...
        {
          const std::string componentType(reinterpret_cast<char *>(cbor_string_handle(imageTypeHandle[jj].value)),
                                          cbor_string_length(imageTypeHandle[jj].value));
        ...
        {
          const std::string pixelType(reinterpret_cast<char *>(cbor_string_handle(imageTypeHandle[jj].value)),
                                      cbor_string_length(imageTypeHandle[jj].value));
...
size_t
WasmImageIO ::WriteCBOR(const void * buffer, unsigned char ** cborBufferPtr, bool allocateCBORBuffer)
{
  cbor_item_t * index = cbor_new_definite_map(7);

  cbor_item_t * imageTypeItem = cbor_new_definite_map(4);
  cbor_map_add(imageTypeItem,
               cbor_pair{ cbor_move(cbor_build_string("dimension")),
  ...
  cbor_map_add(
    imageTypeItem,
    cbor_pair{ cbor_move(cbor_build_string("componentType")), cbor_move(cbor_build_string(componentString.c_str())) });
  const std::string pixelString = WasmPixelTypeFromIOPixelEnum(this->GetPixelType());
  cbor_map_add(
    imageTypeItem,
    cbor_pair{ cbor_move(cbor_build_string("pixelType")), cbor_move(cbor_build_string(pixelString.c_str())) });
  cbor_map_add(imageTypeItem,
               cbor_pair{ cbor_move(cbor_build_string("components")),
                          cbor_move(cbor_build_uint32(this->GetNumberOfComponents())) });
  cbor_map_add(index, cbor_pair{ cbor_move(cbor_build_string("imageType")), cbor_move(imageTypeItem) });
  ...
  cbor_map_add(index, cbor_pair{ cbor_move(cbor_build_string("origin")), cbor_move(originItem) });
  ...
  cbor_map_add(index, cbor_pair{ cbor_move(cbor_build_string("spacing")), cbor_move(spacingItem) });
  ...
  cbor_item_t * directionItem = cbor_build_bytestring(reinterpret_cast<unsigned char *>(&(direction.at(0))),
  ...
  cbor_map_add(index, cbor_pair{ cbor_move(cbor_build_string("direction")), cbor_move(directionTag) });
  ...
  cbor_map_add(index, cbor_pair{ cbor_move(cbor_build_string("size")), cbor_move(sizeItem) });
  ...
  cbor_map_add(index, cbor_pair{ cbor_move(cbor_build_string("metadata")), cbor_move(metaDataItem) });

  if (buffer != nullptr)
  {
    const SizeValueType numberOfBytesToWrite = static_cast<SizeValueType>(this->GetImageSizeInBytes());
    cbor_item_t *       dataItem =
      cbor_build_bytestring(reinterpret_cast<const unsigned char *>(buffer), numberOfBytesToWrite);
    uint64_t tag = 0;
    ...
    cbor_map_add(index, cbor_pair{ cbor_move(cbor_build_string("data")), cbor_move(dataTag) });
  }
...
void
WasmImageIO ::ReadImageInformation()
{
  ...
  if ((cborPos != std::string::npos) && (cborPos == path.length() - 5))
  {
...
bool
WasmImageIO ::CanWriteFile(const char * name)
{
...
void
WasmImageIO ::Write(const void * buffer)
{
  const std::string path(this->GetFileName());
