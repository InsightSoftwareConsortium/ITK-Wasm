...
if (options.copyBuildArtifacts) {
  ...
  try {
    fs.mkdirSync(path.join('dist', 'core', 'web-workers'), { recursive: true })
  } catch (err) {
    if (err.code !== 'EEXIST') throw err
  }
...
if (options.buildTestPipelines) {
  const buildPipeline = async (pipelinePath) => {
    ...
    pipelineFiles = pipelineFiles.concat(glob.sync(path.join(pipelinePath, 'emscripten-build', '*.wasm.zst')))
