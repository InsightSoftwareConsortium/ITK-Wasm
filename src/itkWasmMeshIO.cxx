...
void
WasmMeshIO ::PrintSelf(std::ostream & os, Indent indent) const
{
...
bool
WasmMeshIO ::CanReadFile(const char * filename)
{
...
void
WasmMeshIO ::ReadCBOR(void * buffer, unsigned char * cborBuffer, size_t cborBufferLength)
{
  ...
  for (size_t ii = 0; ii < indexCount; ++ii)
  {
    ...
    if (key == "meshType")
    {
      ...
      {
        const std::string_view meshTypeKey(reinterpret_cast<char *>(cbor_string_handle(meshTypeHandle[jj].key)),
                                           cbor_string_length(meshTypeHandle[jj].key));
        ...
        {
          const std::string pointComponentType(reinterpret_cast<char *>(cbor_string_handle(meshTypeHandle[jj].value)),
                                               cbor_string_length(meshTypeHandle[jj].value));
        ...
        {
          const std::string pointPixelType(reinterpret_cast<char *>(cbor_string_handle(meshTypeHandle[jj].value)),
                                           cbor_string_length(meshTypeHandle[jj].value));
        ...
        else if (meshTypeKey == "pointPixelComponentType")
        {
          const std::string pointPixelComponentType(
            reinterpret_cast<char *>(cbor_string_handle(meshTypeHandle[jj].value)),
            cbor_string_length(meshTypeHandle[jj].value));
        ...
        {
          const std::string cellComponentType(reinterpret_cast<char *>(cbor_string_handle(meshTypeHandle[jj].value)),
                                              cbor_string_length(meshTypeHandle[jj].value));
        ...
        {
          const std::string cellPixelType(reinterpret_cast<char *>(cbor_string_handle(meshTypeHandle[jj].value)),
                                          cbor_string_length(meshTypeHandle[jj].value));
        ...
        {
          const std::string cellPixelComponentType(
            reinterpret_cast<char *>(cbor_string_handle(meshTypeHandle[jj].value)),
            cbor_string_length(meshTypeHandle[jj].value));
...
auto
WasmMeshIO ::GetPointSetJSON() -> PointSetJSON
{
...
void
WasmMeshIO ::SetJSON(const MeshJSON & meshJSON)
{
...
void
WasmMeshIO ::SetJSON(const PointSetJSON & pointSetJSON)
{
...
WasmMeshIO ::WriteCBOR()
{
  ...
  cbor_map_add(
    meshTypeItem,
    cbor_pair{ cbor_move(cbor_build_string("dimension")), cbor_move(cbor_build_uint32(this->GetPointDimension())) });
  std::string componentString = WasmComponentTypeFromIOComponentEnum(this->GetPointComponentType());
  cbor_map_add(meshTypeItem,
               cbor_pair{ cbor_move(cbor_build_string("pointComponentType")),
  ...
  cbor_map_add(
    meshTypeItem,
    cbor_pair{ cbor_move(cbor_build_string("pointPixelType")), cbor_move(cbor_build_string(pixelString.c_str())) });
  componentString = WasmComponentTypeFromIOComponentEnum(this->GetPointPixelComponentType());
  cbor_map_add(meshTypeItem,
               cbor_pair{ cbor_move(cbor_build_string("pointPixelComponentType")),
                          cbor_move(cbor_build_string(componentString.c_str())) });
  cbor_map_add(meshTypeItem,
               cbor_pair{ cbor_move(cbor_build_string("pointPixelComponents")),
  ...
  cbor_map_add(meshTypeItem,
               cbor_pair{ cbor_move(cbor_build_string("cellComponentType")),
                          cbor_move(cbor_build_string(componentString.c_str())) });
  pixelString = WasmPixelTypeFromIOPixelEnum(this->GetCellPixelType());
  cbor_map_add(
    meshTypeItem,
    cbor_pair{ cbor_move(cbor_build_string("cellPixelType")), cbor_move(cbor_build_string(pixelString.c_str())) });
  componentString = WasmComponentTypeFromIOComponentEnum(this->GetCellPixelComponentType());
  cbor_map_add(meshTypeItem,
               cbor_pair{ cbor_move(cbor_build_string("cellPixelComponentType")),
                          cbor_move(cbor_build_string(componentString.c_str())) });
  cbor_map_add(meshTypeItem,
               cbor_pair{ cbor_move(cbor_build_string("cellPixelComponents")),
                          cbor_move(cbor_build_uint32(this->GetNumberOfCellPixelComponents())) });
  cbor_map_add(index, cbor_pair{ cbor_move(cbor_build_string("meshType")), cbor_move(meshTypeItem) });

  cbor_map_add(index,
               cbor_pair{ cbor_move(cbor_build_string("numberOfPoints")),
  ...
  cbor_map_add(index,
               cbor_pair{ cbor_move(cbor_build_string("numberOfPointPixels")),
  ...
  cbor_map_add(
    index,
    cbor_pair{ cbor_move(cbor_build_string("numberOfCells")), cbor_move(cbor_build_uint64(this->GetNumberOfCells())) });
  ...
  cbor_map_add(index,
               cbor_pair{ cbor_move(cbor_build_string("numberOfCellPixels")),
  ...
  cbor_map_add(index,
               cbor_pair{ cbor_move(cbor_build_string("cellBufferSize")),
                          cbor_move(cbor_build_uint64(this->GetCellBufferSize())) });
}

void
WasmMeshIO ::ReadMeshInformation()
{
...
void
WasmMeshIO ::ReadPoints(void * buffer)
{
  const SizeValueType numberOfBytesToBeRead = static_cast<SizeValueType>(
    this->GetNumberOfPoints() * this->GetPointDimension() * ITKComponentSize(this->GetPointComponentType()));
  ...
  {
    itkExceptionMacro(<< "Read failed: Wanted " << numberOfBytesToBeRead << " bytes, but read " << dataStream.gcount()
                      << " bytes.");
  }
}
...
void
WasmMeshIO ::ReadCells(void * buffer)
{
...
bool
WasmMeshIO ::CanWriteFile(const char * name)
{
...
void
WasmMeshIO ::WritePoints(void * buffer)
{
  const SizeValueType numberOfBytes =
    this->GetNumberOfPoints() * this->GetPointDimension() * ITKComponentSize(this->GetPointComponentType());
...
void
WasmMeshIO ::WriteCells(void * buffer)
{
...
void
WasmMeshIO ::WritePointData(void * buffer)
{
...
void
WasmMeshIO ::WriteCellData(void * buffer)
{
  const SizeValueType numberOfBytes = this->GetNumberOfPointPixels() * this->GetNumberOfPointPixelComponents() *
                                      ITKComponentSize(this->GetCellPixelComponentType());
