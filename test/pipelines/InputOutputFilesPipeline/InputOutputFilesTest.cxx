...
{
  itk::wasm::Pipeline pipeline("A pipeline to test general binary and text IO", argc, argv);
  ...
  pipeline.add_option("--input-text-stream", inputTextStream, "The input text stream")->group("Streams");

  itk::wasm::OutputTextStream outputTextStream;
  pipeline.add_option("--output-text-stream", outputTextStream, "The output text stream")->group("Streams");

  itk::wasm::InputBinaryStream inputBinaryStream;
  pipeline.add_option("--input-binary-stream", inputBinaryStream, "The input binary stream")->group("Streams");

  itk::wasm::OutputBinaryStream outputBinaryStream;
  pipeline.add_option("--output-binary-stream", outputBinaryStream, "The output binary stream")->group("Streams");


  std::string inputTextFile;
  pipeline.add_option("--input-text-file", inputTextFile, "The input text file")->group("Files");

  std::string outputTextFile;
  pipeline.add_option("--output-text-file", outputTextFile, "The output text file")->group("Files");

  std::string inputBinaryFile;
  pipeline.add_option("--input-binary-file", inputBinaryFile, "The input binary file")->group("Files");

  std::string outputBinaryFile;
  pipeline.add_option("--output-binary-file", outputBinaryFile, "The output binary file")->group("Files");
