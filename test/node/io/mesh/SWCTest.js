...
const testInputFilePath = path.resolve('build-emscripten', 'ExternalData', 'test', 'Input', '11706c2.CNG.swc')
const testOutputFilePath = path.resolve('build-emscripten', 'Testing', 'Temporary', 'SWCTest-11706c2.CNG.swc')
...
test('readMeshLocalFile reads a SWC file path given on the local filesystem', t => {
  return readMeshLocalFile(testInputFilePath).then(function (mesh) {
    verifyMesh(t, mesh)
  })
})

test('writeMeshLocalFile writes a SWC file path on the local filesystem', (t) => {
