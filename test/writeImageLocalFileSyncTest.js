...
const writeImageLocalFileSync = require(path.resolve(__dirname, '..', 'dist', 'writeImageLocalFileSync.js'))

const testInputFilePath = path.resolve(__dirname, '..', 'build', 'ExternalData', 'test', 'Input', 'cthead1.png')
const testOutputFilePath = path.resolve(__dirname, '..', 'build', 'Testing', 'Temporary', 'writeImageLocalFileSyncTest-cthead1.png')
...
test('writeImageLocalFileSync writes a file path on the local filesystem', t => {
  const image = readImageLocalFileSync(testInputFilePath)
  const useCompression = false
  writeImageLocalFileSync(useCompression, image, testOutputFilePath)
  const readImage = readImageLocalFileSync(testOutputFilePath)
  verifyImage(t, readImage)
})
