...
class WriteImageController {
  ...
  constructor(loadSampleInputs) {
    ...
    this.model = new WriteImageModel()
    const model = this.model

    this.webWorker = null

    if (loadSampleInputs) {
      const loadSampleInputsButton = document.querySelector("#writeImageInputs [name=loadSampleInputs]")
    ...
    // ----------------------------------------------
    // Inputs
    const imageElement = document.querySelector('#writeImageInputs input[name=image-file]')
    ...
    const serializedImageElement = document.querySelector('#writeImageInputs sl-input[name=serialized-image]')
    ...
    // ----------------------------------------------
    // Options
    const informationOnlyElement = document.querySelector('#writeImageInputs sl-checkbox[name=information-only]')
    informationOnlyElement.addEventListener('sl-change', (event) => {
        model.options.set("informationOnly", informationOnlyElement.checked)
    })

    const useCompressionElement = document.querySelector('#writeImageInputs sl-checkbox[name=use-compression]')
    ...
    // ----------------------------------------------
    // Outputs
    const serializedImageOutputDownload = document.querySelector('#writeImageOutputs sl-button[name=serialized-image-download]')
    ...
    const onSelectTab = async (event) => {
      if (event.detail.name === 'writeImage-panel') {
        const params = new URLSearchParams(window.location.search)
        if (!params.has('functionName') || params.get('functionName') !== 'writeImage') {
          params.set('functionName', 'writeImage')
          const url = new URL(document.location)
          url.search = params
          window.history.replaceState({ functionName: 'writeImage' }, '', url)
          await preRun()
        }
      }
    }
    ...
    function onInit() {
      const params = new URLSearchParams(window.location.search)
      if (params.has('functionName') && params.get('functionName') === 'writeImage') {
        tabGroup.show('writeImage-panel')
        preRun()
      }
    }
    onInit()

    const runButton = document.querySelector('#writeImageInputs sl-button[name="run"]')
    runButton.addEventListener('click', async (event) => {
      ...
      try {
        ...
        const { serializedImage, } = await this.run()
        const t1 = performance.now()
        globalThis.notify("writeImage successfully completed", `in ${t1 - t0} milliseconds.`, "success", "rocket-fill")
        ...
        const serializedImageOutput = document.getElementById("writeImage-serialized-image-details")
...
const writeImageController = new WriteImageController(writeImageLoadSampleInputs)
