...
import bioRadReadImageLoadSampleInputs, { usePreRun } from "./bio-rad-read-image-load-sample-inputs.js"

class BioRadReadImageModel {
...
class BioRadReadImageController {

  constructor(loadSampleInputs) {
    this.loadSampleInputs = loadSampleInputs

    this.model = new BioRadReadImageModel()
    const model = this.model

    this.webWorker = null

    if (loadSampleInputs) {
      const loadSampleInputsButton = document.querySelector("#bioRadReadImageInputs [name=loadSampleInputs]")
    ...
    // ----------------------------------------------
    // Inputs
    const serializedImageElement = document.querySelector('#bioRadReadImageInputs input[name=serialized-image-file]')
    serializedImageElement.addEventListener('change', async (event) => {
        ...
        const details = document.getElementById("bioRadReadImage-serialized-image-details")
    ...
    // ----------------------------------------------
    // Options
    const informationOnlyElement = document.querySelector('#bioRadReadImageInputs sl-checkbox[name=information-only]')
    ...
    // ----------------------------------------------
    // Outputs
    const couldReadOutputDownload = document.querySelector('#bioRadReadImageOutputs sl-button[name=could-read-download]')
    ...
    const imageOutputDownload = document.querySelector('#bioRadReadImageOutputs sl-button[name=image-download]')
    ...
    const onSelectTab = async (event) => {
      if (event.detail.name === 'bioRadReadImage-panel') {
        const params = new URLSearchParams(window.location.search)
        if (!params.has('functionName') || params.get('functionName') !== 'bioRadReadImage') {
    ...
    function onInit() {
      const params = new URLSearchParams(window.location.search)
      if (params.has('functionName') && params.get('functionName') === 'bioRadReadImage') {
        tabGroup.show('bioRadReadImage-panel')
        preRun()
      }
    }
    onInit()

    const runButton = document.querySelector('#bioRadReadImageInputs sl-button[name="run"]')
    runButton.addEventListener('click', async (event) => {
      ...
      try {
        ...
        globalThis.notify("bioRadReadImage successfully completed", `in ${t1 - t0} milliseconds.`, "success", "rocket-fill")

        model.outputs.set("couldRead", couldRead)
        couldReadOutputDownload.variant = "success"
        couldReadOutputDownload.disabled = false
        const couldReadDetails = document.getElementById("bioRadReadImage-could-read-details")
        ...
        const couldReadOutput = document.getElementById("bioRadReadImage-could-read-details")
  ...
  async run() {
    const { webWorker, couldRead, image, } = await imageIo.bioRadReadImage(this.webWorker,
...
const bioRadReadImageController = new BioRadReadImageController(bioRadReadImageLoadSampleInputs)
