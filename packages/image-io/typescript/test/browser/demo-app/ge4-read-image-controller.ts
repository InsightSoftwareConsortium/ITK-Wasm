...
import ge4ReadImageLoadSampleInputs, { usePreRun } from "./ge4-read-image-load-sample-inputs.js"

class Ge4ReadImageModel {
...
class Ge4ReadImageController {

  constructor(loadSampleInputs) {
    this.loadSampleInputs = loadSampleInputs

    this.model = new Ge4ReadImageModel()
    const model = this.model

    this.webWorker = null

    if (loadSampleInputs) {
      const loadSampleInputsButton = document.querySelector("#ge4ReadImageInputs [name=loadSampleInputs]")
    ...
    // ----------------------------------------------
    // Inputs
    const serializedImageElement = document.querySelector('#ge4ReadImageInputs input[name=serialized-image-file]')
    serializedImageElement.addEventListener('change', async (event) => {
        ...
        const details = document.getElementById("ge4ReadImage-serialized-image-details")
    ...
    // ----------------------------------------------
    // Options
    const informationOnlyElement = document.querySelector('#ge4ReadImageInputs sl-checkbox[name=information-only]')
    ...
    // ----------------------------------------------
    // Outputs
    const couldReadOutputDownload = document.querySelector('#ge4ReadImageOutputs sl-button[name=could-read-download]')
    ...
    const imageOutputDownload = document.querySelector('#ge4ReadImageOutputs sl-button[name=image-download]')
    ...
    const onSelectTab = async (event) => {
      if (event.detail.name === 'ge4ReadImage-panel') {
        const params = new URLSearchParams(window.location.search)
        if (!params.has('functionName') || params.get('functionName') !== 'ge4ReadImage') {
          params.set('functionName', 'ge4ReadImage')
          const url = new URL(document.location)
          url.search = params
          window.history.replaceState({ functionName: 'ge4ReadImage' }, '', url)
          await preRun()
        }
      }
    }
    ...
    function onInit() {
      const params = new URLSearchParams(window.location.search)
      if (params.has('functionName') && params.get('functionName') === 'ge4ReadImage') {
        tabGroup.show('ge4ReadImage-panel')
        preRun()
      }
    }
    onInit()

    const runButton = document.querySelector('#ge4ReadImageInputs sl-button[name="run"]')
    runButton.addEventListener('click', async (event) => {
      ...
      try {
        ...
        globalThis.notify("ge4ReadImage successfully completed", `in ${t1 - t0} milliseconds.`, "success", "rocket-fill")

        model.outputs.set("couldRead", couldRead)
        couldReadOutputDownload.variant = "success"
        couldReadOutputDownload.disabled = false
        const couldReadDetails = document.getElementById("ge4ReadImage-could-read-details")
        ...
        const couldReadOutput = document.getElementById("ge4ReadImage-could-read-details")
  ...
  async run() {
    const { webWorker, couldRead, image, } = await imageIo.ge4ReadImage(this.webWorker,
...
const ge4ReadImageController = new Ge4ReadImageController(ge4ReadImageLoadSampleInputs)
