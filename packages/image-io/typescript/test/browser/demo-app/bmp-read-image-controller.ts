...
class BmpReadImageController {

  constructor(loadSampleInputs) {
    this.loadSampleInputs = loadSampleInputs

    this.model = new BmpReadImageModel()
    const model = this.model

    this.webWorker = null

    if (loadSampleInputs) {
      const loadSampleInputsButton = document.querySelector("#bmpReadImageInputs [name=loadSampleInputs]")
    ...
    // ----------------------------------------------
    // Inputs
    const serializedImageElement = document.querySelector('#bmpReadImageInputs input[name=serialized-image-file]')
    serializedImageElement.addEventListener('change', async (event) => {
        ...
        const details = document.getElementById("bmpReadImage-serialized-image-details")
    ...
    // ----------------------------------------------
    // Options
    const informationOnlyElement = document.querySelector('#bmpReadImageInputs sl-checkbox[name=information-only]')
    ...
    // ----------------------------------------------
    // Outputs
    const couldReadOutputDownload = document.querySelector('#bmpReadImageOutputs sl-button[name=could-read-download]')
    ...
    const imageOutputDownload = document.querySelector('#bmpReadImageOutputs sl-button[name=image-download]')
    ...
    const onSelectTab = async (event) => {
      if (event.detail.name === 'bmpReadImage-panel') {
        const params = new URLSearchParams(window.location.search)
        if (!params.has('functionName') || params.get('functionName') !== 'bmpReadImage') {
          params.set('functionName', 'bmpReadImage')
          const url = new URL(document.location)
          url.search = params
          window.history.replaceState({ functionName: 'bmpReadImage' }, '', url)
          await preRun()
        }
      }
    }
    ...
    function onInit() {
      const params = new URLSearchParams(window.location.search)
      if (params.has('functionName') && params.get('functionName') === 'bmpReadImage') {
        tabGroup.show('bmpReadImage-panel')
        preRun()
      }
    }
    onInit()

    const runButton = document.querySelector('#bmpReadImageInputs sl-button[name="run"]')
    runButton.addEventListener('click', async (event) => {
      ...
      try {
        ...
        globalThis.notify("bmpReadImage successfully completed", `in ${t1 - t0} milliseconds.`, "success", "rocket-fill")

        model.outputs.set("couldRead", couldRead)
        couldReadOutputDownload.variant = "success"
        couldReadOutputDownload.disabled = false
        const couldReadDetails = document.getElementById("bmpReadImage-could-read-details")
        ...
        const couldReadOutput = document.getElementById("bmpReadImage-could-read-details")

        model.outputs.set("image", image)
        imageOutputDownload.variant = "success"
        imageOutputDownload.disabled = false
        const imageDetails = document.getElementById("bmpReadImage-image-details")
        ...
        const imageOutput = document.getElementById('bmpReadImage-image-details')
      } catch (error) {
...
const bmpReadImageController = new BmpReadImageController(bmpReadImageLoadSampleInputs)
