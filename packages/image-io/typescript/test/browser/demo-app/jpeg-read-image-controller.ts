...
import jpegReadImageLoadSampleInputs, { usePreRun } from "./jpeg-read-image-load-sample-inputs.js"

class JpegReadImageModel {
...
class JpegReadImageController {

  constructor(loadSampleInputs) {
    this.loadSampleInputs = loadSampleInputs

    this.model = new JpegReadImageModel()
    const model = this.model

    this.webWorker = null

    if (loadSampleInputs) {
      const loadSampleInputsButton = document.querySelector("#jpegReadImageInputs [name=loadSampleInputs]")
    ...
    // ----------------------------------------------
    // Inputs
    const serializedImageElement = document.querySelector('#jpegReadImageInputs input[name=serialized-image-file]')
    serializedImageElement.addEventListener('change', async (event) => {
        ...
        const details = document.getElementById("jpegReadImage-serialized-image-details")
    ...
    // ----------------------------------------------
    // Options
    const informationOnlyElement = document.querySelector('#jpegReadImageInputs sl-checkbox[name=information-only]')
    ...
    // ----------------------------------------------
    // Outputs
    const couldReadOutputDownload = document.querySelector('#jpegReadImageOutputs sl-button[name=could-read-download]')
    ...
    const imageOutputDownload = document.querySelector('#jpegReadImageOutputs sl-button[name=image-download]')
    ...
    const onSelectTab = async (event) => {
      if (event.detail.name === 'jpegReadImage-panel') {
        const params = new URLSearchParams(window.location.search)
        if (!params.has('functionName') || params.get('functionName') !== 'jpegReadImage') {
          params.set('functionName', 'jpegReadImage')
          const url = new URL(document.location)
          url.search = params
          window.history.replaceState({ functionName: 'jpegReadImage' }, '', url)
          await preRun()
        }
      }
    }
    ...
    function onInit() {
      const params = new URLSearchParams(window.location.search)
      if (params.has('functionName') && params.get('functionName') === 'jpegReadImage') {
        tabGroup.show('jpegReadImage-panel')
        preRun()
      }
    }
    onInit()

    const runButton = document.querySelector('#jpegReadImageInputs sl-button[name="run"]')
    runButton.addEventListener('click', async (event) => {
      ...
      try {
        ...
        globalThis.notify("jpegReadImage successfully completed", `in ${t1 - t0} milliseconds.`, "success", "rocket-fill")

        model.outputs.set("couldRead", couldRead)
        couldReadOutputDownload.variant = "success"
        couldReadOutputDownload.disabled = false
        const couldReadDetails = document.getElementById("jpegReadImage-could-read-details")
        ...
        const couldReadOutput = document.getElementById("jpegReadImage-could-read-details")

        model.outputs.set("image", image)
        imageOutputDownload.variant = "success"
        imageOutputDownload.disabled = false
        const imageDetails = document.getElementById("jpegReadImage-image-details")
        ...
        const imageOutput = document.getElementById('jpegReadImage-image-details')
      } catch (error) {
  ...
  async run() {
    const { webWorker, couldRead, image, } = await imageIo.jpegReadImage(this.webWorker,
...
const jpegReadImageController = new JpegReadImageController(jpegReadImageLoadSampleInputs)
