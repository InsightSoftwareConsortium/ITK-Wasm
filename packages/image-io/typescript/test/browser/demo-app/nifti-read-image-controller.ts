...
import niftiReadImageLoadSampleInputs, { usePreRun } from "./nifti-read-image-load-sample-inputs.js"

class NiftiReadImageModel {
...
class NiftiReadImageController {

  constructor(loadSampleInputs) {
    this.loadSampleInputs = loadSampleInputs

    this.model = new NiftiReadImageModel()
    const model = this.model

    this.webWorker = null

    if (loadSampleInputs) {
      const loadSampleInputsButton = document.querySelector("#niftiReadImageInputs [name=loadSampleInputs]")
    ...
    // ----------------------------------------------
    // Inputs
    const serializedImageElement = document.querySelector('#niftiReadImageInputs input[name=serialized-image-file]')
    serializedImageElement.addEventListener('change', async (event) => {
        ...
        const details = document.getElementById("niftiReadImage-serialized-image-details")
    ...
    // ----------------------------------------------
    // Options
    const informationOnlyElement = document.querySelector('#niftiReadImageInputs sl-checkbox[name=information-only]')
    ...
    // ----------------------------------------------
    // Outputs
    const couldReadOutputDownload = document.querySelector('#niftiReadImageOutputs sl-button[name=could-read-download]')
    ...
    const imageOutputDownload = document.querySelector('#niftiReadImageOutputs sl-button[name=image-download]')
    ...
    const onSelectTab = async (event) => {
      if (event.detail.name === 'niftiReadImage-panel') {
        const params = new URLSearchParams(window.location.search)
        if (!params.has('functionName') || params.get('functionName') !== 'niftiReadImage') {
          params.set('functionName', 'niftiReadImage')
          const url = new URL(document.location)
          url.search = params
          window.history.replaceState({ functionName: 'niftiReadImage' }, '', url)
          await preRun()
        }
      }
    }
    ...
    function onInit() {
      const params = new URLSearchParams(window.location.search)
      if (params.has('functionName') && params.get('functionName') === 'niftiReadImage') {
        tabGroup.show('niftiReadImage-panel')
        preRun()
      }
    }
    onInit()

    const runButton = document.querySelector('#niftiReadImageInputs sl-button[name="run"]')
    runButton.addEventListener('click', async (event) => {
      ...
      try {
        ...
        globalThis.notify("niftiReadImage successfully completed", `in ${t1 - t0} milliseconds.`, "success", "rocket-fill")

        model.outputs.set("couldRead", couldRead)
        couldReadOutputDownload.variant = "success"
        couldReadOutputDownload.disabled = false
        const couldReadDetails = document.getElementById("niftiReadImage-could-read-details")
        ...
        const couldReadOutput = document.getElementById("niftiReadImage-could-read-details")

        model.outputs.set("image", image)
        imageOutputDownload.variant = "success"
        imageOutputDownload.disabled = false
        const imageDetails = document.getElementById("niftiReadImage-image-details")
        ...
        const imageOutput = document.getElementById('niftiReadImage-image-details')
      } catch (error) {
  ...
  async run() {
    const { webWorker, couldRead, image, } = await imageIo.niftiReadImage(this.webWorker,
...
const niftiReadImageController = new NiftiReadImageController(niftiReadImageLoadSampleInputs)
