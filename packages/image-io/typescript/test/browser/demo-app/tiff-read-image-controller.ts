...
import tiffReadImageLoadSampleInputs, { usePreRun } from "./tiff-read-image-load-sample-inputs.js"

class TiffReadImageModel {
...
class TiffReadImageController {

  constructor(loadSampleInputs) {
    this.loadSampleInputs = loadSampleInputs

    this.model = new TiffReadImageModel()
    const model = this.model

    this.webWorker = null

    if (loadSampleInputs) {
      const loadSampleInputsButton = document.querySelector("#tiffReadImageInputs [name=loadSampleInputs]")
    ...
    // ----------------------------------------------
    // Inputs
    const serializedImageElement = document.querySelector('#tiffReadImageInputs input[name=serialized-image-file]')
    serializedImageElement.addEventListener('change', async (event) => {
        ...
        const details = document.getElementById("tiffReadImage-serialized-image-details")
    ...
    // ----------------------------------------------
    // Options
    const informationOnlyElement = document.querySelector('#tiffReadImageInputs sl-checkbox[name=information-only]')
    ...
    // ----------------------------------------------
    // Outputs
    const couldReadOutputDownload = document.querySelector('#tiffReadImageOutputs sl-button[name=could-read-download]')
    ...
    const imageOutputDownload = document.querySelector('#tiffReadImageOutputs sl-button[name=image-download]')
    ...
    const onSelectTab = async (event) => {
      if (event.detail.name === 'tiffReadImage-panel') {
        const params = new URLSearchParams(window.location.search)
        if (!params.has('functionName') || params.get('functionName') !== 'tiffReadImage') {
          params.set('functionName', 'tiffReadImage')
          const url = new URL(document.location)
          url.search = params
          window.history.replaceState({ functionName: 'tiffReadImage' }, '', url)
          await preRun()
        }
      }
    }
    ...
    function onInit() {
      const params = new URLSearchParams(window.location.search)
      if (params.has('functionName') && params.get('functionName') === 'tiffReadImage') {
        tabGroup.show('tiffReadImage-panel')
        preRun()
      }
    }
    onInit()

    const runButton = document.querySelector('#tiffReadImageInputs sl-button[name="run"]')
    runButton.addEventListener('click', async (event) => {
      ...
      try {
        ...
        globalThis.notify("tiffReadImage successfully completed", `in ${t1 - t0} milliseconds.`, "success", "rocket-fill")

        model.outputs.set("couldRead", couldRead)
        couldReadOutputDownload.variant = "success"
        couldReadOutputDownload.disabled = false
        const couldReadDetails = document.getElementById("tiffReadImage-could-read-details")
        ...
        const couldReadOutput = document.getElementById("tiffReadImage-could-read-details")

        model.outputs.set("image", image)
        imageOutputDownload.variant = "success"
        imageOutputDownload.disabled = false
        const imageDetails = document.getElementById("tiffReadImage-image-details")
        ...
        const imageOutput = document.getElementById('tiffReadImage-image-details')
      } catch (error) {
  ...
  async run() {
    const { webWorker, couldRead, image, } = await imageIo.tiffReadImage(this.webWorker,
...
const tiffReadImageController = new TiffReadImageController(tiffReadImageLoadSampleInputs)
