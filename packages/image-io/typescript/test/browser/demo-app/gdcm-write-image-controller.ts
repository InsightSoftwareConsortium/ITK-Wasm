...
import gdcmWriteImageLoadSampleInputs, { usePreRun } from "./gdcm-write-image-load-sample-inputs.js"

class GdcmWriteImageModel {
...
class GdcmWriteImageController {

  constructor(loadSampleInputs) {
    this.loadSampleInputs = loadSampleInputs

    this.model = new GdcmWriteImageModel()
    const model = this.model

    this.webWorker = null

    if (loadSampleInputs) {
      const loadSampleInputsButton = document.querySelector("#gdcmWriteImageInputs [name=loadSampleInputs]")
    ...
    // ----------------------------------------------
    // Inputs
    const imageElement = document.querySelector('#gdcmWriteImageInputs input[name=image-file]')
    imageElement.addEventListener('change', async (event) => {
        ...
        const details = document.getElementById("gdcmWriteImage-image-details")
        details.innerHTML = `<pre>${globalThis.escapeHtml(JSON.stringify(image, globalThis.interfaceTypeJsonReplacer, 2))}</pre>`
        details.disabled = false
    })

    const serializedImageElement = document.querySelector('#gdcmWriteImageInputs sl-input[name=serialized-image]')
    ...
    // ----------------------------------------------
    // Options
    const informationOnlyElement = document.querySelector('#gdcmWriteImageInputs sl-checkbox[name=information-only]')
    informationOnlyElement.addEventListener('sl-change', (event) => {
        model.options.set("informationOnly", informationOnlyElement.checked)
    })

    const useCompressionElement = document.querySelector('#gdcmWriteImageInputs sl-checkbox[name=use-compression]')
    ...
    // ----------------------------------------------
    // Outputs
    const couldWriteOutputDownload = document.querySelector('#gdcmWriteImageOutputs sl-button[name=could-write-download]')
    ...
    const serializedImageOutputDownload = document.querySelector('#gdcmWriteImageOutputs sl-button[name=serialized-image-download]')
    ...
    const onSelectTab = async (event) => {
      if (event.detail.name === 'gdcmWriteImage-panel') {
        const params = new URLSearchParams(window.location.search)
        if (!params.has('functionName') || params.get('functionName') !== 'gdcmWriteImage') {
          params.set('functionName', 'gdcmWriteImage')
          const url = new URL(document.location)
          url.search = params
          window.history.replaceState({ functionName: 'gdcmWriteImage' }, '', url)
          await preRun()
        }
      }
    }
    ...
    function onInit() {
      const params = new URLSearchParams(window.location.search)
      if (params.has('functionName') && params.get('functionName') === 'gdcmWriteImage') {
        tabGroup.show('gdcmWriteImage-panel')
        preRun()
      }
    }
    onInit()

    const runButton = document.querySelector('#gdcmWriteImageInputs sl-button[name="run"]')
    runButton.addEventListener('click', async (event) => {
      ...
      try {
        ...
        globalThis.notify("gdcmWriteImage successfully completed", `in ${t1 - t0} milliseconds.`, "success", "rocket-fill")

        model.outputs.set("couldWrite", couldWrite)
        couldWriteOutputDownload.variant = "success"
        couldWriteOutputDownload.disabled = false
        const couldWriteDetails = document.getElementById("gdcmWriteImage-could-write-details")
        ...
        const couldWriteOutput = document.getElementById("gdcmWriteImage-could-write-details")
        ...
        const serializedImageOutput = document.getElementById("gdcmWriteImage-serialized-image-details")
  ...
  async run() {
    const { webWorker, couldWrite, serializedImage, } = await imageIo.gdcmWriteImage(this.webWorker,
...
const gdcmWriteImageController = new GdcmWriteImageController(gdcmWriteImageLoadSampleInputs)
