...
import tiffWriteImageLoadSampleInputs, { usePreRun } from "./tiff-write-image-load-sample-inputs.js"

class TiffWriteImageModel {
...
class TiffWriteImageController {

  constructor(loadSampleInputs) {
    this.loadSampleInputs = loadSampleInputs

    this.model = new TiffWriteImageModel()
    const model = this.model

    this.webWorker = null

    if (loadSampleInputs) {
      const loadSampleInputsButton = document.querySelector("#tiffWriteImageInputs [name=loadSampleInputs]")
    ...
    // ----------------------------------------------
    // Inputs
    const imageElement = document.querySelector('#tiffWriteImageInputs input[name=image-file]')
    imageElement.addEventListener('change', async (event) => {
        ...
        const details = document.getElementById("tiffWriteImage-image-details")
        details.innerHTML = `<pre>${globalThis.escapeHtml(JSON.stringify(image, globalThis.interfaceTypeJsonReplacer, 2))}</pre>`
        details.disabled = false
    })

    const serializedImageElement = document.querySelector('#tiffWriteImageInputs sl-input[name=serialized-image]')
    ...
    // ----------------------------------------------
    // Options
    const informationOnlyElement = document.querySelector('#tiffWriteImageInputs sl-checkbox[name=information-only]')
    informationOnlyElement.addEventListener('sl-change', (event) => {
        model.options.set("informationOnly", informationOnlyElement.checked)
    })

    const useCompressionElement = document.querySelector('#tiffWriteImageInputs sl-checkbox[name=use-compression]')
    ...
    // ----------------------------------------------
    // Outputs
    const couldWriteOutputDownload = document.querySelector('#tiffWriteImageOutputs sl-button[name=could-write-download]')
    ...
    const serializedImageOutputDownload = document.querySelector('#tiffWriteImageOutputs sl-button[name=serialized-image-download]')
    ...
    const onSelectTab = async (event) => {
      if (event.detail.name === 'tiffWriteImage-panel') {
        const params = new URLSearchParams(window.location.search)
        if (!params.has('functionName') || params.get('functionName') !== 'tiffWriteImage') {
          params.set('functionName', 'tiffWriteImage')
          const url = new URL(document.location)
          url.search = params
          window.history.replaceState({ functionName: 'tiffWriteImage' }, '', url)
          await preRun()
        }
      }
    }
    ...
    function onInit() {
      const params = new URLSearchParams(window.location.search)
      if (params.has('functionName') && params.get('functionName') === 'tiffWriteImage') {
        tabGroup.show('tiffWriteImage-panel')
        preRun()
      }
    }
    onInit()

    const runButton = document.querySelector('#tiffWriteImageInputs sl-button[name="run"]')
    runButton.addEventListener('click', async (event) => {
      ...
      try {
        ...
        globalThis.notify("tiffWriteImage successfully completed", `in ${t1 - t0} milliseconds.`, "success", "rocket-fill")

        model.outputs.set("couldWrite", couldWrite)
        couldWriteOutputDownload.variant = "success"
        couldWriteOutputDownload.disabled = false
        const couldWriteDetails = document.getElementById("tiffWriteImage-could-write-details")
        ...
        const couldWriteOutput = document.getElementById("tiffWriteImage-could-write-details")
        ...
        const serializedImageOutput = document.getElementById("tiffWriteImage-serialized-image-details")
  ...
  async run() {
    const { webWorker, couldWrite, serializedImage, } = await imageIo.tiffWriteImage(this.webWorker,
