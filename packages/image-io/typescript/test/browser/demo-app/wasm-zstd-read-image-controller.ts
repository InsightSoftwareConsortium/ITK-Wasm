...
import wasmZstdReadImageLoadSampleInputs, { usePreRun } from "./wasm-zstd-read-image-load-sample-inputs.js"

class WasmZstdReadImageModel {
...
class WasmZstdReadImageController {

  constructor(loadSampleInputs) {
    this.loadSampleInputs = loadSampleInputs

    this.model = new WasmZstdReadImageModel()
    const model = this.model

    this.webWorker = null

    if (loadSampleInputs) {
      const loadSampleInputsButton = document.querySelector("#wasmZstdReadImageInputs [name=loadSampleInputs]")
    ...
    // ----------------------------------------------
    // Inputs
    const serializedImageElement = document.querySelector('#wasmZstdReadImageInputs input[name=serialized-image-file]')
    serializedImageElement.addEventListener('change', async (event) => {
        ...
        const details = document.getElementById("wasmZstdReadImage-serialized-image-details")
    ...
    // ----------------------------------------------
    // Options
    const informationOnlyElement = document.querySelector('#wasmZstdReadImageInputs sl-checkbox[name=information-only]')
    ...
    // ----------------------------------------------
    // Outputs
    const couldReadOutputDownload = document.querySelector('#wasmZstdReadImageOutputs sl-button[name=could-read-download]')
    ...
    const imageOutputDownload = document.querySelector('#wasmZstdReadImageOutputs sl-button[name=image-download]')
    ...
    const onSelectTab = async (event) => {
      if (event.detail.name === 'wasmZstdReadImage-panel') {
        const params = new URLSearchParams(window.location.search)
        if (!params.has('functionName') || params.get('functionName') !== 'wasmZstdReadImage') {
          params.set('functionName', 'wasmZstdReadImage')
          const url = new URL(document.location)
          url.search = params
          window.history.replaceState({ functionName: 'wasmZstdReadImage' }, '', url)
          await preRun()
        }
      }
    }
    ...
    function onInit() {
      const params = new URLSearchParams(window.location.search)
      if (params.has('functionName') && params.get('functionName') === 'wasmZstdReadImage') {
        tabGroup.show('wasmZstdReadImage-panel')
        preRun()
      }
    }
    onInit()

    const runButton = document.querySelector('#wasmZstdReadImageInputs sl-button[name="run"]')
    runButton.addEventListener('click', async (event) => {
      ...
      try {
        ...
        globalThis.notify("wasmZstdReadImage successfully completed", `in ${t1 - t0} milliseconds.`, "success", "rocket-fill")

        model.outputs.set("couldRead", couldRead)
        couldReadOutputDownload.variant = "success"
        couldReadOutputDownload.disabled = false
        const couldReadDetails = document.getElementById("wasmZstdReadImage-could-read-details")
        ...
        const couldReadOutput = document.getElementById("wasmZstdReadImage-could-read-details")

        model.outputs.set("image", image)
        imageOutputDownload.variant = "success"
        imageOutputDownload.disabled = false
        const imageDetails = document.getElementById("wasmZstdReadImage-image-details")
        ...
        const imageOutput = document.getElementById('wasmZstdReadImage-image-details')
      } catch (error) {
  ...
  async run() {
    const { webWorker, couldRead, image, } = await imageIo.wasmZstdReadImage(this.webWorker,
...
const wasmZstdReadImageController = new WasmZstdReadImageController(wasmZstdReadImageLoadSampleInputs)
