...
import metaWriteImageLoadSampleInputs, { usePreRun } from "./meta-write-image-load-sample-inputs.js"

class MetaWriteImageModel {
...
class MetaWriteImageController {

  constructor(loadSampleInputs) {
    this.loadSampleInputs = loadSampleInputs

    this.model = new MetaWriteImageModel()
    const model = this.model

    this.webWorker = null

    if (loadSampleInputs) {
      const loadSampleInputsButton = document.querySelector("#metaWriteImageInputs [name=loadSampleInputs]")
    ...
    // ----------------------------------------------
    // Inputs
    const imageElement = document.querySelector('#metaWriteImageInputs input[name=image-file]')
    imageElement.addEventListener('change', async (event) => {
        ...
        const details = document.getElementById("metaWriteImage-image-details")
        details.innerHTML = `<pre>${globalThis.escapeHtml(JSON.stringify(image, globalThis.interfaceTypeJsonReplacer, 2))}</pre>`
        details.disabled = false
    })

    const serializedImageElement = document.querySelector('#metaWriteImageInputs sl-input[name=serialized-image]')
    ...
    // ----------------------------------------------
    // Options
    const informationOnlyElement = document.querySelector('#metaWriteImageInputs sl-checkbox[name=information-only]')
    informationOnlyElement.addEventListener('sl-change', (event) => {
        model.options.set("informationOnly", informationOnlyElement.checked)
    })

    const useCompressionElement = document.querySelector('#metaWriteImageInputs sl-checkbox[name=use-compression]')
    ...
    // ----------------------------------------------
    // Outputs
    const couldWriteOutputDownload = document.querySelector('#metaWriteImageOutputs sl-button[name=could-write-download]')
    ...
    const serializedImageOutputDownload = document.querySelector('#metaWriteImageOutputs sl-button[name=serialized-image-download]')
    ...
    const onSelectTab = async (event) => {
      if (event.detail.name === 'metaWriteImage-panel') {
        const params = new URLSearchParams(window.location.search)
        if (!params.has('functionName') || params.get('functionName') !== 'metaWriteImage') {
          params.set('functionName', 'metaWriteImage')
          const url = new URL(document.location)
          url.search = params
          window.history.replaceState({ functionName: 'metaWriteImage' }, '', url)
          await preRun()
        }
      }
    }
    ...
    function onInit() {
      const params = new URLSearchParams(window.location.search)
      if (params.has('functionName') && params.get('functionName') === 'metaWriteImage') {
        tabGroup.show('metaWriteImage-panel')
        preRun()
      }
    }
    onInit()

    const runButton = document.querySelector('#metaWriteImageInputs sl-button[name="run"]')
    runButton.addEventListener('click', async (event) => {
      ...
      try {
        ...
        globalThis.notify("metaWriteImage successfully completed", `in ${t1 - t0} milliseconds.`, "success", "rocket-fill")

        model.outputs.set("couldWrite", couldWrite)
        couldWriteOutputDownload.variant = "success"
        couldWriteOutputDownload.disabled = false
        const couldWriteDetails = document.getElementById("metaWriteImage-could-write-details")
        ...
        const couldWriteOutput = document.getElementById("metaWriteImage-could-write-details")
        ...
        const serializedImageOutput = document.getElementById("metaWriteImage-serialized-image-details")
  ...
  async run() {
    const { webWorker, couldWrite, serializedImage, } = await imageIo.metaWriteImage(this.webWorker,
...
const metaWriteImageController = new MetaWriteImageController(metaWriteImageLoadSampleInputs)
