...
import pngWriteImageLoadSampleInputs, { usePreRun } from "./png-write-image-load-sample-inputs.js"

class PngWriteImageModel {
...
class PngWriteImageController {

  constructor(loadSampleInputs) {
    this.loadSampleInputs = loadSampleInputs

    this.model = new PngWriteImageModel()
    const model = this.model

    this.webWorker = null

    if (loadSampleInputs) {
      const loadSampleInputsButton = document.querySelector("#pngWriteImageInputs [name=loadSampleInputs]")
    ...
    // ----------------------------------------------
    // Inputs
    const imageElement = document.querySelector('#pngWriteImageInputs input[name=image-file]')
    imageElement.addEventListener('change', async (event) => {
        ...
        const details = document.getElementById("pngWriteImage-image-details")
        details.innerHTML = `<pre>${globalThis.escapeHtml(JSON.stringify(image, globalThis.interfaceTypeJsonReplacer, 2))}</pre>`
        details.disabled = false
    })

    const serializedImageElement = document.querySelector('#pngWriteImageInputs sl-input[name=serialized-image]')
    ...
    // ----------------------------------------------
    // Options
    const informationOnlyElement = document.querySelector('#pngWriteImageInputs sl-checkbox[name=information-only]')
    informationOnlyElement.addEventListener('sl-change', (event) => {
        model.options.set("informationOnly", informationOnlyElement.checked)
    })

    const useCompressionElement = document.querySelector('#pngWriteImageInputs sl-checkbox[name=use-compression]')
    ...
    // ----------------------------------------------
    // Outputs
    const couldWriteOutputDownload = document.querySelector('#pngWriteImageOutputs sl-button[name=could-write-download]')
    ...
    const serializedImageOutputDownload = document.querySelector('#pngWriteImageOutputs sl-button[name=serialized-image-download]')
    ...
    const onSelectTab = async (event) => {
      if (event.detail.name === 'pngWriteImage-panel') {
        const params = new URLSearchParams(window.location.search)
        if (!params.has('functionName') || params.get('functionName') !== 'pngWriteImage') {
          params.set('functionName', 'pngWriteImage')
          const url = new URL(document.location)
          url.search = params
          window.history.replaceState({ functionName: 'pngWriteImage' }, '', url)
          await preRun()
        }
      }
    }
    ...
    function onInit() {
      const params = new URLSearchParams(window.location.search)
      if (params.has('functionName') && params.get('functionName') === 'pngWriteImage') {
        tabGroup.show('pngWriteImage-panel')
        preRun()
      }
    }
    onInit()

    const runButton = document.querySelector('#pngWriteImageInputs sl-button[name="run"]')
    runButton.addEventListener('click', async (event) => {
      ...
      try {
        ...
        globalThis.notify("pngWriteImage successfully completed", `in ${t1 - t0} milliseconds.`, "success", "rocket-fill")

        model.outputs.set("couldWrite", couldWrite)
        couldWriteOutputDownload.variant = "success"
        couldWriteOutputDownload.disabled = false
        const couldWriteDetails = document.getElementById("pngWriteImage-could-write-details")
        ...
        const couldWriteOutput = document.getElementById("pngWriteImage-could-write-details")
        ...
        const serializedImageOutput = document.getElementById("pngWriteImage-serialized-image-details")
  ...
  async run() {
    const { webWorker, couldWrite, serializedImage, } = await imageIo.pngWriteImage(this.webWorker,
...
const pngWriteImageController = new PngWriteImageController(pngWriteImageLoadSampleInputs)
