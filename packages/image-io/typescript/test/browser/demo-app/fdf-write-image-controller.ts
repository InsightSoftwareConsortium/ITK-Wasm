...
import fdfWriteImageLoadSampleInputs, { usePreRun } from "./fdf-write-image-load-sample-inputs.js"

class FdfWriteImageModel {
...
class FdfWriteImageController {

  constructor(loadSampleInputs) {
    this.loadSampleInputs = loadSampleInputs

    this.model = new FdfWriteImageModel()
    const model = this.model

    this.webWorker = null

    if (loadSampleInputs) {
      const loadSampleInputsButton = document.querySelector("#fdfWriteImageInputs [name=loadSampleInputs]")
    ...
    // ----------------------------------------------
    // Inputs
    const imageElement = document.querySelector('#fdfWriteImageInputs input[name=image-file]')
    imageElement.addEventListener('change', async (event) => {
        ...
        const details = document.getElementById("fdfWriteImage-image-details")
        details.innerHTML = `<pre>${globalThis.escapeHtml(JSON.stringify(image, globalThis.interfaceTypeJsonReplacer, 2))}</pre>`
        details.disabled = false
    })

    const serializedImageElement = document.querySelector('#fdfWriteImageInputs sl-input[name=serialized-image]')
    ...
    // ----------------------------------------------
    // Options
    const informationOnlyElement = document.querySelector('#fdfWriteImageInputs sl-checkbox[name=information-only]')
    informationOnlyElement.addEventListener('sl-change', (event) => {
        model.options.set("informationOnly", informationOnlyElement.checked)
    })

    const useCompressionElement = document.querySelector('#fdfWriteImageInputs sl-checkbox[name=use-compression]')
    ...
    // ----------------------------------------------
    // Outputs
    const couldWriteOutputDownload = document.querySelector('#fdfWriteImageOutputs sl-button[name=could-write-download]')
    ...
    const serializedImageOutputDownload = document.querySelector('#fdfWriteImageOutputs sl-button[name=serialized-image-download]')
    ...
    const onSelectTab = async (event) => {
      if (event.detail.name === 'fdfWriteImage-panel') {
        const params = new URLSearchParams(window.location.search)
        if (!params.has('functionName') || params.get('functionName') !== 'fdfWriteImage') {
          params.set('functionName', 'fdfWriteImage')
          const url = new URL(document.location)
          url.search = params
          window.history.replaceState({ functionName: 'fdfWriteImage' }, '', url)
          await preRun()
        }
      }
    }
    ...
    function onInit() {
      const params = new URLSearchParams(window.location.search)
      if (params.has('functionName') && params.get('functionName') === 'fdfWriteImage') {
        tabGroup.show('fdfWriteImage-panel')
        preRun()
      }
    }
    onInit()

    const runButton = document.querySelector('#fdfWriteImageInputs sl-button[name="run"]')
    runButton.addEventListener('click', async (event) => {
      ...
      try {
        ...
        globalThis.notify("fdfWriteImage successfully completed", `in ${t1 - t0} milliseconds.`, "success", "rocket-fill")

        model.outputs.set("couldWrite", couldWrite)
        couldWriteOutputDownload.variant = "success"
        couldWriteOutputDownload.disabled = false
        const couldWriteDetails = document.getElementById("fdfWriteImage-could-write-details")
        ...
        const couldWriteOutput = document.getElementById("fdfWriteImage-could-write-details")
        ...
        const serializedImageOutput = document.getElementById("fdfWriteImage-serialized-image-details")
  ...
  async run() {
    const { webWorker, couldWrite, serializedImage, } = await imageIo.fdfWriteImage(this.webWorker,
...
const fdfWriteImageController = new FdfWriteImageController(fdfWriteImageLoadSampleInputs)
