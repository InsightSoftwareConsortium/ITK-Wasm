...
import { fdfReadImageNode } from '../../dist/bundles/image-io-node.js'
import { FloatTypes, PixelTypes, getMatrixElement } from 'itk-wasm'

import { testInputPath } from './common.js'

const testInputFilePath = path.join(testInputPath, 'test.fdf')

test('Test reading a FDF file', async t => {
  const { couldRead, image } = await fdfReadImageNode(testInputFilePath)
  t.true(couldRead)
