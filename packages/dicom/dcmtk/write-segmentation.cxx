...
int
runPipeline(const itk::wasm::InputImage<ScalarImageType> & inputImage,
            ...
            const bool                                     useLabelIDAsSegmentNumber)
{
  ...
  */
  ...
  segmentation files!" << endl; cerr << "segmentAttributes has: " << metaRoot["segmentAttributes"].size() << " items,
  the are " << segImageFiles.size() << " input segmentation files!" << endl; return EXIT_FAILURE;
    }
  }
  */
...
main(int argc, char * argv[])
{
  ...
  pipeline.add_option("seg-image", inputImage, "dicom segmentation object as an image")
    ->required()
  ...
  return runPipeline(
    inputImage, metaInfo, refDicomSeriesFiles, outputDicomFile, skipEmptySlices, useLabelIDAsSegmentNumber);
}
