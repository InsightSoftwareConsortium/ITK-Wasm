...
Itk_wasm_dcmtk_rt_study::save_rtss(const char * fname, const ItkWasmRtStudyMetadata & metadata)
{
  ...
  /* ----------------------------------------------------------------- */
  dataset->putAndInsertOFStringArray(DCM_InstanceCreationDate, d_ptr->rt_study_metadata->get_study_date());
  dataset->putAndInsertOFStringArray(DCM_InstanceCreationTime, d_ptr->rt_study_metadata->get_study_time());
  dataset->putAndInsertOFStringArray(DCM_InstanceCreatorUID, PlmUidPrefix::getInstance().get().c_str());
  dataset->putAndInsertString(DCM_SOPClassUID, UID_RTStructureSetStorage);
  dcmtk_put(dataset, DCM_SOPInstanceUID, d_ptr->rt_study_metadata->get_rtstruct_instance_uid());
  dataset->putAndInsertOFStringArray(DCM_StudyDate, d_ptr->rt_study_metadata->get_study_date());
  dataset->putAndInsertOFStringArray(DCM_StudyTime, d_ptr->rt_study_metadata->get_study_time());
  dcmtk_copy_from_metadata(dataset, rtstruct_metadata, DCM_StudyDescription, "");
  ...
  dcmtk_copy_from_metadata(dataset, rtstruct_metadata, DCM_SeriesDescription, "");
  ...
  dataset->putAndInsertString(DCM_SoftwareVersions, metadata.softwareVersions.c_str());

  dataset->putAndInsertString(DCM_StudyInstanceUID, d_ptr->rt_study_metadata->get_study_uid().c_str());
  dataset->putAndInsertString(DCM_SeriesInstanceUID, d_ptr->rt_study_metadata->get_rtstruct_series_uid());
  ...
  dataset->putAndInsertOFStringArray(DCM_StructureSetDate, d_ptr->rt_study_metadata->get_study_date());
  dataset->putAndInsertOFStringArray(DCM_StructureSetTime, d_ptr->rt_study_metadata->get_study_time());
  ...
  DcmItem *            rfor_item = 0;
  dataset->findOrCreateSequenceItem(DCM_ReferencedFrameOfReferenceSequence, rfor_item, -2);
  rfor_item->putAndInsertString(DCM_FrameOfReferenceUID,
                                d_ptr->rt_study_metadata->get_frame_of_reference_uid().c_str());
  dataset->findAndGetSequence(DCM_ReferencedFrameOfReferenceSequence, rfor_seq);
  DcmItem * rtrstudy_item = 0;
  rfor_item->findOrCreateSequenceItem(DCM_RTReferencedStudySequence, rtrstudy_item, -2);
  rtrstudy_item->putAndInsertString(DCM_ReferencedSOPClassUID, UID_RETIRED_StudyComponentManagementSOPClass);
  rtrstudy_item->putAndInsertString(DCM_ReferencedSOPInstanceUID, d_ptr->rt_study_metadata->get_study_uid().c_str());
  DcmItem * rtrseries_item = 0;
  rtrstudy_item->findOrCreateSequenceItem(DCM_RTReferencedSeriesSequence, rtrseries_item, -2);
  rtrseries_item->putAndInsertString(DCM_SeriesInstanceUID, d_ptr->rt_study_metadata->get_ct_series_uid());

  for (int k = 0; k < d_ptr->rt_study_metadata->num_slices(); k++)
  {
    DcmItem * ci_item = 0;
    rtrseries_item->findOrCreateSequenceItem(DCM_ContourImageSequence, ci_item, -2);
    ci_item->putAndInsertString(DCM_ReferencedSOPClassUID, UID_CTImageStorage);
    ci_item->putAndInsertString(DCM_ReferencedSOPInstanceUID, d_ptr->rt_study_metadata->get_slice_uid(k));
  }
  ...
  for (size_t i = 0; i < rtss->num_structures; i++)
  {
    ...
    std::string tmp;
    dataset->findOrCreateSequenceItem(DCM_StructureSetROISequence, ssroi_item, -2);
    ...
    ssroi_item->putAndInsertString(DCM_ReferencedFrameOfReferenceUID,
                                   d_ptr->rt_study_metadata->get_frame_of_reference_uid().c_str());
    ssroi_item->putAndInsertString(DCM_ROIName, rtss->slist[i]->name.c_str());
    if (metadata.roiMetadata.size() > i)
    {
      ...
      ssroi_item->putAndInsertString(DCM_ROIDescription, metadata.roiMetadata[i].roiDescription.c_str());
    }
    else
    {
      ssroi_item->putAndInsertString(DCM_ROIGenerationAlgorithm, "");
  ...
  for (size_t i = 0; i < rtss->num_structures; i++)
  {
    ...
    DcmItem *  roic_item = 0;
    dataset->findOrCreateSequenceItem(DCM_ROIContourSequence, roic_item, -2);
    ...
    for (size_t j = 0; j < curr_structure->num_contours; j++)
    {
      ...
      DcmItem * c_item = 0;
      roic_item->findOrCreateSequenceItem(DCM_ContourSequence, c_item, -2);

      /* ContourImageSequence */
      if (curr_contour->ct_slice_uid != "")
      {
        DcmItem * ci_item = 0;
        c_item->findOrCreateSequenceItem(DCM_ContourImageSequence, ci_item, -2);
        ci_item->putAndInsertString(DCM_ReferencedSOPClassUID, UID_CTImageStorage);
        ci_item->putAndInsertString(DCM_ReferencedSOPInstanceUID, curr_contour->ct_slice_uid.c_str());
      }

      /* ContourGeometricType */
      c_item->putAndInsertString(DCM_ContourGeometricType, "CLOSED_PLANAR");
      ...
      /* ContourData */
      tmp = string_format("%.8g\\%.8g\\%.8g", curr_contour->x[0], curr_contour->y[0], curr_contour->z[0]);
      for (size_t k = 1; k < curr_contour->num_vertices; k++)
      {
        std::string tmp2 =
          string_format("\\%.8g\\%.8g\\%.8g", curr_contour->x[k], curr_contour->y[k], curr_contour->z[k]);
        tmp += tmp2;
      }
  ...
  for (size_t i = 0; i < rtss->num_structures; i++)
  {
    ...
    dataset->findOrCreateSequenceItem(DCM_RTROIObservationsSequence, rtroio_item, -2);
    ...
    if (metadata.roiMetadata.size() > i)
    {
      rtroio_item->putAndInsertString(DCM_RTROIInterpretedType, metadata.roiMetadata[i].rtRoiInterpretedType.c_str());
    }
    else
    {
    ...
    if (rtss->slist[i]->rsp_value > -1.0)
    {
      ...
      if (rtroio_item->findOrCreateSequenceItem(DCM_ROIPhysicalPropertiesSequence, rsp_item, -2).good())
      {
