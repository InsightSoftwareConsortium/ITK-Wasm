...
int
runPipeline(const itk::wasm::InputImage<VectorImageType> & inputImage,
  ...
  if (metaRoot.isMember("segmentAttributes"))
  {
    if (metaRoot["segmentAttributes"].size() != segmentations.size())
    {
      ...
      cerr << "segmentAttributes has: " << metaRoot["segmentAttributes"].size() << " items, the are "
           << segmentations.size() << " input segmentation files!" << endl;
      return EXIT_FAILURE;
    }
  }
...
main(int argc, char * argv[])
{
  itk::wasm::Pipeline pipeline(
    "write-overlapping-segmentation", "Write DICOM segmentation object for overlapping segments.", argc, argv);
