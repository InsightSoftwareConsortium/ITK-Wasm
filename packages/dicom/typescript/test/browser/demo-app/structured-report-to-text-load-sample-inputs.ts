export default async function structuredReportToTextLoadSampleInputs(model, preRun=false) {
  const dicomButton = document.querySelector('#structuredReportToTextInputs sl-button[name=dicom-file-file-button]')
  if (!preRun) {
  ...
  if (!preRun) {
    const dicomElement = document.getElementById('structuredReportToText-dicom-file-details')
