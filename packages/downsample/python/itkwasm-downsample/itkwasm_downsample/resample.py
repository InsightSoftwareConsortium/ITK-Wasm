...
def resample(
    ...
    :rtype:  Image
    """
    func = environment_dispatch("itkwasm_downsample", "resample")
    output = func(input, size=size, output_spacing=output_spacing, output_origin=output_origin, output_direction=output_direction, transform=transform, interpolator=interpolator, default_value=default_value)
    return output
