  ...
  ~OutputMeshIO()
  {
    ...
    else
    {
      ...
      if (!this->m_MeshIO.IsNull() && !this->m_Identifier.empty())
      {
        ...
        numberOfBytes = static_cast<SizeValueType>(this->m_MeshIO->GetCellBufferSize() *
                                                   ITKComponentSize(this->m_MeshIO->GetCellComponentType()));
        if (numberOfBytes)
        {
        ...
        numberOfBytes = static_cast<SizeValueType>(this->m_MeshIO->GetNumberOfCellPixels() *
                                                   this->m_MeshIO->GetNumberOfCellPixelComponents() *
                                                   ITKComponentSize(this->m_MeshIO->GetCellPixelComponentType()));
        if (numberOfBytes)
        {
          loadBuffer.resize(numberOfBytes);
...
bool
lexical_cast(const std::string & input, OutputMeshIO & outputMeshIO)
{
