...
import stlWriteMeshLoadSampleInputs, { usePreRun } from "./stl-write-mesh-load-sample-inputs.js"

class StlWriteMeshModel {
...
class StlWriteMeshController {

  constructor(loadSampleInputs) {
    this.loadSampleInputs = loadSampleInputs

    this.model = new StlWriteMeshModel()
    const model = this.model

    this.webWorker = null

    if (loadSampleInputs) {
      const loadSampleInputsButton = document.querySelector("#stlWriteMeshInputs [name=loadSampleInputs]")
    ...
    // ----------------------------------------------
    // Inputs
    const meshElement = document.querySelector('#stlWriteMeshInputs input[name=mesh-file]')
    meshElement.addEventListener('change', async (event) => {
        ...
        const details = document.getElementById("stlWriteMesh-mesh-details")
        details.innerHTML = `<pre>${globalThis.escapeHtml(JSON.stringify(mesh, globalThis.interfaceTypeJsonReplacer, 2))}</pre>`
        details.disabled = false
    })

    const serializedMeshElement = document.querySelector('#stlWriteMeshInputs sl-input[name=serialized-mesh]')
    ...
    // ----------------------------------------------
    // Options
    const informationOnlyElement = document.querySelector('#stlWriteMeshInputs sl-checkbox[name=information-only]')
    informationOnlyElement.addEventListener('sl-change', (event) => {
        model.options.set("informationOnly", informationOnlyElement.checked)
    })

    const useCompressionElement = document.querySelector('#stlWriteMeshInputs sl-checkbox[name=use-compression]')
    useCompressionElement.addEventListener('sl-change', (event) => {
        model.options.set("useCompression", useCompressionElement.checked)
    })

    const binaryFileTypeElement = document.querySelector('#stlWriteMeshInputs sl-checkbox[name=binary-file-type]')
    ...
    // ----------------------------------------------
    // Outputs
    const couldWriteOutputDownload = document.querySelector('#stlWriteMeshOutputs sl-button[name=could-write-download]')
    ...
    const serializedMeshOutputDownload = document.querySelector('#stlWriteMeshOutputs sl-button[name=serialized-mesh-download]')
    ...
    const onSelectTab = async (event) => {
      if (event.detail.name === 'stlWriteMesh-panel') {
        const params = new URLSearchParams(window.location.search)
        if (!params.has('functionName') || params.get('functionName') !== 'stlWriteMesh') {
          params.set('functionName', 'stlWriteMesh')
          const url = new URL(document.location)
          url.search = params
          window.history.replaceState({ functionName: 'stlWriteMesh' }, '', url)
          await preRun()
        }
      }
    }
    ...
    function onInit() {
      const params = new URLSearchParams(window.location.search)
      if (params.has('functionName') && params.get('functionName') === 'stlWriteMesh') {
        tabGroup.show('stlWriteMesh-panel')
        preRun()
      }
    }
    onInit()

    const runButton = document.querySelector('#stlWriteMeshInputs sl-button[name="run"]')
    runButton.addEventListener('click', async (event) => {
      ...
      try {
        ...
        globalThis.notify("stlWriteMesh successfully completed", `in ${t1 - t0} milliseconds.`, "success", "rocket-fill")

        model.outputs.set("couldWrite", couldWrite)
        couldWriteOutputDownload.variant = "success"
        couldWriteOutputDownload.disabled = false
        const couldWriteDetails = document.getElementById("stlWriteMesh-could-write-details")
        ...
        const couldWriteOutput = document.getElementById("stlWriteMesh-could-write-details")
        ...
        const serializedMeshOutput = document.getElementById("stlWriteMesh-serialized-mesh-details")
  ...
  async run() {
    ...
    const { webWorker, couldWrite, serializedMesh, } = await meshIo.stlWriteMesh(      this.model.inputs.get('mesh'),
...
const stlWriteMeshController = new StlWriteMeshController(stlWriteMeshLoadSampleInputs)
