...
class SwcReadMeshController {
  ...
  async run() {
    ...
    const { webWorker, couldRead, mesh, } = await meshIo.swcReadMesh(      { data: this.model.inputs.get('serializedMesh').data.slice(), path: this.model.inputs.get('serializedMesh').path },
