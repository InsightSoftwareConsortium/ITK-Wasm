...
import swcReadMeshLoadSampleInputs, { usePreRun } from "./swc-read-mesh-load-sample-inputs.js"

class SwcReadMeshModel {
...
class SwcReadMeshController {

  constructor(loadSampleInputs) {
    this.loadSampleInputs = loadSampleInputs

    this.model = new SwcReadMeshModel()
    const model = this.model

    if (loadSampleInputs) {
      const loadSampleInputsButton = document.querySelector("#swcReadMeshInputs [name=loadSampleInputs]")
    ...
    // ----------------------------------------------
    // Inputs
    const serializedMeshElement = document.querySelector('#swcReadMeshInputs input[name=serialized-mesh-file]')
    serializedMeshElement.addEventListener('change', async (event) => {
        ...
        const details = document.getElementById("swcReadMesh-serialized-mesh-details")
    ...
    // ----------------------------------------------
    // Options
    const informationOnlyElement = document.querySelector('#swcReadMeshInputs sl-checkbox[name=information-only]')
    ...
    // ----------------------------------------------
    // Outputs
    const couldReadOutputDownload = document.querySelector('#swcReadMeshOutputs sl-button[name=could-read-download]')
    ...
    const meshOutputDownload = document.querySelector('#swcReadMeshOutputs sl-button[name=mesh-download]')
    meshOutputDownload.addEventListener('click', async (event) => {
        event.preventDefault()
        event.stopPropagation()
        if (model.outputs.has("mesh")) {
            const meshDownloadFormat = document.getElementById('swcReadMesh-mesh-output-format')
    ...
    const onSelectTab = async (event) => {
      if (event.detail.name === 'swcReadMesh-panel') {
        const params = new URLSearchParams(window.location.search)
        if (!params.has('functionName') || params.get('functionName') !== 'swcReadMesh') {
          params.set('functionName', 'swcReadMesh')
          const url = new URL(document.location)
          url.search = params
          window.history.replaceState({ functionName: 'swcReadMesh' }, '', url)
          await preRun()
        }
      }
    }
    ...
    function onInit() {
      const params = new URLSearchParams(window.location.search)
      if (params.has('functionName') && params.get('functionName') === 'swcReadMesh') {
        tabGroup.show('swcReadMesh-panel')
        preRun()
      }
    }
    onInit()

    const runButton = document.querySelector('#swcReadMeshInputs sl-button[name="run"]')
    runButton.addEventListener('click', async (event) => {
      ...
      try {
        ...
        globalThis.notify("swcReadMesh successfully completed", `in ${t1 - t0} milliseconds.`, "success", "rocket-fill")

        model.outputs.set("couldRead", couldRead)
        couldReadOutputDownload.variant = "success"
        couldReadOutputDownload.disabled = false
        const couldReadDetails = document.getElementById("swcReadMesh-could-read-details")
        ...
        const couldReadOutput = document.getElementById("swcReadMesh-could-read-details")
  ...
  async run() {
    ...
    const { couldRead, mesh, } = await meshIo.swcReadMesh(      { data: this.model.inputs.get('serializedMesh').data.slice(), path: this.model.inputs.get('serializedMesh').path },
...
const swcReadMeshController = new SwcReadMeshController(swcReadMeshLoadSampleInputs)
