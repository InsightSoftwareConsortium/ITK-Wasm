...
import swcWriteMeshLoadSampleInputs, { usePreRun } from "./swc-write-mesh-load-sample-inputs.js"

class SwcWriteMeshModel {
...
class SwcWriteMeshController {

  constructor(loadSampleInputs) {
    this.loadSampleInputs = loadSampleInputs

    this.model = new SwcWriteMeshModel()
    const model = this.model

    this.webWorker = null

    if (loadSampleInputs) {
      const loadSampleInputsButton = document.querySelector("#swcWriteMeshInputs [name=loadSampleInputs]")
    ...
    // ----------------------------------------------
    // Inputs
    const meshElement = document.querySelector('#swcWriteMeshInputs input[name=mesh-file]')
    meshElement.addEventListener('change', async (event) => {
        ...
        const details = document.getElementById("swcWriteMesh-mesh-details")
        details.innerHTML = `<pre>${globalThis.escapeHtml(JSON.stringify(mesh, globalThis.interfaceTypeJsonReplacer, 2))}</pre>`
        details.disabled = false
    })

    const serializedMeshElement = document.querySelector('#swcWriteMeshInputs sl-input[name=serialized-mesh]')
    ...
    // ----------------------------------------------
    // Options
    const informationOnlyElement = document.querySelector('#swcWriteMeshInputs sl-checkbox[name=information-only]')
    informationOnlyElement.addEventListener('sl-change', (event) => {
        model.options.set("informationOnly", informationOnlyElement.checked)
    })

    const useCompressionElement = document.querySelector('#swcWriteMeshInputs sl-checkbox[name=use-compression]')
    useCompressionElement.addEventListener('sl-change', (event) => {
        model.options.set("useCompression", useCompressionElement.checked)
    })

    const binaryFileTypeElement = document.querySelector('#swcWriteMeshInputs sl-checkbox[name=binary-file-type]')
    ...
    // ----------------------------------------------
    // Outputs
    const couldWriteOutputDownload = document.querySelector('#swcWriteMeshOutputs sl-button[name=could-write-download]')
    ...
    const serializedMeshOutputDownload = document.querySelector('#swcWriteMeshOutputs sl-button[name=serialized-mesh-download]')
    ...
    const onSelectTab = async (event) => {
      if (event.detail.name === 'swcWriteMesh-panel') {
        const params = new URLSearchParams(window.location.search)
        if (!params.has('functionName') || params.get('functionName') !== 'swcWriteMesh') {
          params.set('functionName', 'swcWriteMesh')
          const url = new URL(document.location)
          url.search = params
          window.history.replaceState({ functionName: 'swcWriteMesh' }, '', url)
          await preRun()
        }
      }
    }
    ...
    function onInit() {
      const params = new URLSearchParams(window.location.search)
      if (params.has('functionName') && params.get('functionName') === 'swcWriteMesh') {
        tabGroup.show('swcWriteMesh-panel')
        preRun()
      }
    }
    onInit()

    const runButton = document.querySelector('#swcWriteMeshInputs sl-button[name="run"]')
    runButton.addEventListener('click', async (event) => {
      ...
      try {
        ...
        globalThis.notify("swcWriteMesh successfully completed", `in ${t1 - t0} milliseconds.`, "success", "rocket-fill")

        model.outputs.set("couldWrite", couldWrite)
        couldWriteOutputDownload.variant = "success"
        couldWriteOutputDownload.disabled = false
        const couldWriteDetails = document.getElementById("swcWriteMesh-could-write-details")
        ...
        const couldWriteOutput = document.getElementById("swcWriteMesh-could-write-details")
        ...
        const serializedMeshOutput = document.getElementById("swcWriteMesh-serialized-mesh-details")
  ...
  async run() {
    ...
    const { webWorker, couldWrite, serializedMesh, } = await meshIo.swcWriteMesh(      this.model.inputs.get('mesh'),
...
const swcWriteMeshController = new SwcWriteMeshController(swcWriteMeshLoadSampleInputs)
