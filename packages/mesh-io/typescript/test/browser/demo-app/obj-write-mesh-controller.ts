...
import objWriteMeshLoadSampleInputs, { usePreRun } from "./obj-write-mesh-load-sample-inputs.js"

class ObjWriteMeshModel {
...
class ObjWriteMeshController {

  constructor(loadSampleInputs) {
    this.loadSampleInputs = loadSampleInputs

    this.model = new ObjWriteMeshModel()
    const model = this.model

    this.webWorker = null

    if (loadSampleInputs) {
      const loadSampleInputsButton = document.querySelector("#objWriteMeshInputs [name=loadSampleInputs]")
    ...
    // ----------------------------------------------
    // Inputs
    const meshElement = document.querySelector('#objWriteMeshInputs input[name=mesh-file]')
    meshElement.addEventListener('change', async (event) => {
        ...
        const details = document.getElementById("objWriteMesh-mesh-details")
        details.innerHTML = `<pre>${globalThis.escapeHtml(JSON.stringify(mesh, globalThis.interfaceTypeJsonReplacer, 2))}</pre>`
        details.disabled = false
    })

    const serializedMeshElement = document.querySelector('#objWriteMeshInputs sl-input[name=serialized-mesh]')
    ...
    // ----------------------------------------------
    // Options
    const informationOnlyElement = document.querySelector('#objWriteMeshInputs sl-checkbox[name=information-only]')
    informationOnlyElement.addEventListener('sl-change', (event) => {
        model.options.set("informationOnly", informationOnlyElement.checked)
    })

    const useCompressionElement = document.querySelector('#objWriteMeshInputs sl-checkbox[name=use-compression]')
    useCompressionElement.addEventListener('sl-change', (event) => {
        model.options.set("useCompression", useCompressionElement.checked)
    })

    const binaryFileTypeElement = document.querySelector('#objWriteMeshInputs sl-checkbox[name=binary-file-type]')
    ...
    // ----------------------------------------------
    // Outputs
    const couldWriteOutputDownload = document.querySelector('#objWriteMeshOutputs sl-button[name=could-write-download]')
    ...
    const serializedMeshOutputDownload = document.querySelector('#objWriteMeshOutputs sl-button[name=serialized-mesh-download]')
    ...
    const onSelectTab = async (event) => {
      if (event.detail.name === 'objWriteMesh-panel') {
        const params = new URLSearchParams(window.location.search)
        if (!params.has('functionName') || params.get('functionName') !== 'objWriteMesh') {
          params.set('functionName', 'objWriteMesh')
          const url = new URL(document.location)
          url.search = params
          window.history.replaceState({ functionName: 'objWriteMesh' }, '', url)
          await preRun()
        }
      }
    }
    ...
    function onInit() {
      const params = new URLSearchParams(window.location.search)
      if (params.has('functionName') && params.get('functionName') === 'objWriteMesh') {
        tabGroup.show('objWriteMesh-panel')
        preRun()
      }
    }
    onInit()

    const runButton = document.querySelector('#objWriteMeshInputs sl-button[name="run"]')
    runButton.addEventListener('click', async (event) => {
      ...
      try {
        ...
        globalThis.notify("objWriteMesh successfully completed", `in ${t1 - t0} milliseconds.`, "success", "rocket-fill")

        model.outputs.set("couldWrite", couldWrite)
        couldWriteOutputDownload.variant = "success"
        couldWriteOutputDownload.disabled = false
        const couldWriteDetails = document.getElementById("objWriteMesh-could-write-details")
        ...
        const couldWriteOutput = document.getElementById("objWriteMesh-could-write-details")
        ...
        const serializedMeshOutput = document.getElementById("objWriteMesh-serialized-mesh-details")
  ...
  async run() {
    ...
    const { webWorker, couldWrite, serializedMesh, } = await meshIo.objWriteMesh(      this.model.inputs.get('mesh'),
...
const objWriteMeshController = new ObjWriteMeshController(objWriteMeshLoadSampleInputs)
