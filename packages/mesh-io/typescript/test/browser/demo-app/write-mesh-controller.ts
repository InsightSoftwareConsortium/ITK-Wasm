...
class WriteMeshController {

  constructor(loadSampleInputs) {
    this.loadSampleInputs = loadSampleInputs

    this.model = new WriteMeshModel()
    const model = this.model

    this.webWorker = null

    if (loadSampleInputs) {
      const loadSampleInputsButton = document.querySelector("#writeMeshInputs [name=loadSampleInputs]")
    ...
    // ----------------------------------------------
    // Inputs
    const meshElement = document.querySelector('#writeMeshInputs input[name=mesh-file]')
    ...
    const serializedMeshElement = document.querySelector('#writeMeshInputs sl-input[name=serialized-mesh]')
    ...
    // ----------------------------------------------
    // Options
    const informationOnlyElement = document.querySelector('#writeMeshInputs sl-checkbox[name=information-only]')
    informationOnlyElement.addEventListener('sl-change', (event) => {
        model.options.set("informationOnly", informationOnlyElement.checked)
    })

    const useCompressionElement = document.querySelector('#writeMeshInputs sl-checkbox[name=use-compression]')
    useCompressionElement.addEventListener('sl-change', (event) => {
        model.options.set("useCompression", useCompressionElement.checked)
    })

    const binaryFileTypeElement = document.querySelector('#writeMeshInputs sl-checkbox[name=binary-file-type]')
    ...
    // ----------------------------------------------
    // Outputs
    const couldWriteOutputDownload = document.querySelector('#writeMeshOutputs sl-button[name=could-write-download]')
    ...
    const serializedMeshOutputDownload = document.querySelector('#writeMeshOutputs sl-button[name=serialized-mesh-download]')
    ...
    const onSelectTab = async (event) => {
      if (event.detail.name === 'writeMesh-panel') {
        const params = new URLSearchParams(window.location.search)
        if (!params.has('functionName') || params.get('functionName') !== 'writeMesh') {
          params.set('functionName', 'writeMesh')
          const url = new URL(document.location)
          url.search = params
          window.history.replaceState({ functionName: 'writeMesh' }, '', url)
          await preRun()
        }
      }
    }
    ...
    function onInit() {
      const params = new URLSearchParams(window.location.search)
      if (params.has('functionName') && params.get('functionName') === 'writeMesh') {
        tabGroup.show('writeMesh-panel')
        preRun()
      }
    }
    onInit()

    const runButton = document.querySelector('#writeMeshInputs sl-button[name="run"]')
    runButton.addEventListener('click', async (event) => {
      ...
      try {
        ...
        globalThis.notify("writeMesh successfully completed", `in ${t1 - t0} milliseconds.`, "success", "rocket-fill")

        model.outputs.set("couldWrite", couldWrite)
        couldWriteOutputDownload.variant = "success"
        couldWriteOutputDownload.disabled = false
        const couldWriteDetails = document.getElementById("writeMesh-could-write-details")
        ...
        const couldWriteOutput = document.getElementById("writeMesh-could-write-details")
        ...
        const serializedMeshOutput = document.getElementById("writeMesh-serialized-mesh-details")
...
const writeMeshController = new WriteMeshController(writeMeshLoadSampleInputs)
