...
import wasmWritePointSetLoadSampleInputs, { usePreRun } from "./wasm-write-point-set-load-sample-inputs.js"

class WasmWritePointSetModel {
...
class WasmWritePointSetController {

  constructor(loadSampleInputs) {
    this.loadSampleInputs = loadSampleInputs

    this.model = new WasmWritePointSetModel()
    const model = this.model

    if (loadSampleInputs) {
      const loadSampleInputsButton = document.querySelector("#wasmWritePointSetInputs [name=loadSampleInputs]")
    ...
    // ----------------------------------------------
    // Inputs
    const pointSetElement = document.querySelector('#wasmWritePointSetInputs input[name=point-set-file]')
    pointSetElement.addEventListener('change', async (event) => {
        ...
        const details = document.getElementById("wasmWritePointSet-point-set-details")
        details.innerHTML = `<pre>${globalThis.escapeHtml(JSON.stringify(pointSet, globalThis.interfaceTypeJsonReplacer, 2))}</pre>`
        details.disabled = false
    })

    const serializedPointSetElement = document.querySelector('#wasmWritePointSetInputs sl-input[name=serialized-point-set]')
    ...
    // ----------------------------------------------
    // Options
    const informationOnlyElement = document.querySelector('#wasmWritePointSetInputs sl-checkbox[name=information-only]')
    informationOnlyElement.addEventListener('sl-change', (event) => {
        model.options.set("informationOnly", informationOnlyElement.checked)
    })

    const useCompressionElement = document.querySelector('#wasmWritePointSetInputs sl-checkbox[name=use-compression]')
    useCompressionElement.addEventListener('sl-change', (event) => {
        model.options.set("useCompression", useCompressionElement.checked)
    })

    const binaryFileTypeElement = document.querySelector('#wasmWritePointSetInputs sl-checkbox[name=binary-file-type]')
    ...
    // ----------------------------------------------
    // Outputs
    const couldWriteOutputDownload = document.querySelector('#wasmWritePointSetOutputs sl-button[name=could-write-download]')
    ...
    const serializedPointSetOutputDownload = document.querySelector('#wasmWritePointSetOutputs sl-button[name=serialized-point-set-download]')
    ...
    const onSelectTab = async (event) => {
      if (event.detail.name === 'wasmWritePointSet-panel') {
        const params = new URLSearchParams(window.location.search)
        if (!params.has('functionName') || params.get('functionName') !== 'wasmWritePointSet') {
          params.set('functionName', 'wasmWritePointSet')
          const url = new URL(document.location)
          url.search = params
          window.history.replaceState({ functionName: 'wasmWritePointSet' }, '', url)
          await preRun()
        }
      }
    }
    ...
    function onInit() {
      const params = new URLSearchParams(window.location.search)
      if (params.has('functionName') && params.get('functionName') === 'wasmWritePointSet') {
        tabGroup.show('wasmWritePointSet-panel')
        preRun()
      }
    }
    onInit()

    const runButton = document.querySelector('#wasmWritePointSetInputs sl-button[name="run"]')
    runButton.addEventListener('click', async (event) => {
      ...
      try {
        ...
        globalThis.notify("wasmWritePointSet successfully completed", `in ${t1 - t0} milliseconds.`, "success", "rocket-fill")

        model.outputs.set("couldWrite", couldWrite)
        couldWriteOutputDownload.variant = "success"
        couldWriteOutputDownload.disabled = false
        const couldWriteDetails = document.getElementById("wasmWritePointSet-could-write-details")
        ...
        const couldWriteOutput = document.getElementById("wasmWritePointSet-could-write-details")
        ...
        const serializedPointSetOutput = document.getElementById("wasmWritePointSet-serialized-point-set-details")
  ...
  async run() {
    ...
    const { couldWrite, serializedPointSet, } = await meshIo.wasmWritePointSet(      this.model.inputs.get('pointSet'),
...
const wasmWritePointSetController = new WasmWritePointSetController(wasmWritePointSetLoadSampleInputs)
