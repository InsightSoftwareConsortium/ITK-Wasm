...
class VtkPolyDataReadMeshController {
  ...
  async run() {
    ...
    const { webWorker, couldRead, mesh, } = await meshIo.vtkPolyDataReadMesh(      { data: this.model.inputs.get('serializedMesh').data.slice(), path: this.model.inputs.get('serializedMesh').path },
