...
class VtkPolyDataReadMeshController {
  ...
  constructor(loadSampleInputs) {
    ...
    meshOutputDownload.addEventListener('click', async (event) => {
        ...
        if (model.outputs.has("mesh")) {
            const meshDownloadFormat = document.getElementById('vtkPolyDataReadMesh-mesh-output-format')
  ...
  async run() {
    ...
    const { couldRead, mesh, } = await meshIo.vtkPolyDataReadMesh(      { data: this.model.inputs.get('serializedMesh').data.slice(), path: this.model.inputs.get('serializedMesh').path },
