...
class StlReadMeshController {
  ...
  constructor(loadSampleInputs) {
    ...
    runButton.addEventListener('click', async (event) => {
      ...
      try {
        ...
        const meshOutput = document.getElementById('stlReadMesh-mesh-details')
      } catch (error) {
  ...
  async run() {
    ...
    const { webWorker, couldRead, mesh, } = await meshIo.stlReadMesh(      { data: this.model.inputs.get('serializedMesh').data.slice(), path: this.model.inputs.get('serializedMesh').path },
