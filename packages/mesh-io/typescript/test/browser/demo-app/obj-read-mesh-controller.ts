...
class ObjReadMeshController {
  ...
  async run() {
    ...
    const { webWorker, couldRead, mesh, } = await meshIo.objReadMesh(      { data: this.model.inputs.get('serializedMesh').data.slice(), path: this.model.inputs.get('serializedMesh').path },
