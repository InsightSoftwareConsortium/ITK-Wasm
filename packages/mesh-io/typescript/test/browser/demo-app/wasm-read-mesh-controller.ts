...
class WasmReadMeshController {

  constructor(loadSampleInputs) {
    this.loadSampleInputs = loadSampleInputs

    this.model = new WasmReadMeshModel()
    const model = this.model

    if (loadSampleInputs) {
      const loadSampleInputsButton = document.querySelector("#wasmReadMeshInputs [name=loadSampleInputs]")
    ...
    // ----------------------------------------------
    // Inputs
    const serializedMeshElement = document.querySelector('#wasmReadMeshInputs input[name=serialized-mesh-file]')
    serializedMeshElement.addEventListener('change', async (event) => {
        ...
        const details = document.getElementById("wasmReadMesh-serialized-mesh-details")
    ...
    // ----------------------------------------------
    // Options
    const informationOnlyElement = document.querySelector('#wasmReadMeshInputs sl-checkbox[name=information-only]')
    ...
    // ----------------------------------------------
    // Outputs
    const couldReadOutputDownload = document.querySelector('#wasmReadMeshOutputs sl-button[name=could-read-download]')
    ...
    const meshOutputDownload = document.querySelector('#wasmReadMeshOutputs sl-button[name=mesh-download]')
    meshOutputDownload.addEventListener('click', async (event) => {
        event.preventDefault()
        event.stopPropagation()
        if (model.outputs.has("mesh")) {
            const meshDownloadFormat = document.getElementById('wasmReadMesh-mesh-output-format')
    ...
    const onSelectTab = async (event) => {
      if (event.detail.name === 'wasmReadMesh-panel') {
        const params = new URLSearchParams(window.location.search)
        if (!params.has('functionName') || params.get('functionName') !== 'wasmReadMesh') {
          params.set('functionName', 'wasmReadMesh')
          const url = new URL(document.location)
          url.search = params
          window.history.replaceState({ functionName: 'wasmReadMesh' }, '', url)
          await preRun()
        }
      }
    }
    ...
    function onInit() {
      const params = new URLSearchParams(window.location.search)
      if (params.has('functionName') && params.get('functionName') === 'wasmReadMesh') {
        tabGroup.show('wasmReadMesh-panel')
        preRun()
      }
    }
    onInit()

    const runButton = document.querySelector('#wasmReadMeshInputs sl-button[name="run"]')
    runButton.addEventListener('click', async (event) => {
      ...
      try {
        ...
        globalThis.notify("wasmReadMesh successfully completed", `in ${t1 - t0} milliseconds.`, "success", "rocket-fill")

        model.outputs.set("couldRead", couldRead)
        couldReadOutputDownload.variant = "success"
        couldReadOutputDownload.disabled = false
        const couldReadDetails = document.getElementById("wasmReadMesh-could-read-details")
        ...
        const couldReadOutput = document.getElementById("wasmReadMesh-could-read-details")
  ...
  async run() {
    ...
    const { couldRead, mesh, } = await meshIo.wasmReadMesh(      { data: this.model.inputs.get('serializedMesh').data.slice(), path: this.model.inputs.get('serializedMesh').path },
...
const wasmReadMeshController = new WasmReadMeshController(wasmReadMeshLoadSampleInputs)
