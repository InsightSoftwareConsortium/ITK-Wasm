...
class WasmReadMeshController {
  ...
  async run() {
    ...
    const { webWorker, couldRead, mesh, } = await meshIo.wasmReadMesh(      { data: this.model.inputs.get('serializedMesh').data.slice(), path: this.model.inputs.get('serializedMesh').path },
