...
class ByuReadMeshController {
  ...
  async run() {
    ...
    const { webWorker, couldRead, mesh, } = await meshIo.byuReadMesh(      { data: this.model.inputs.get('serializedMesh').data.slice(), path: this.model.inputs.get('serializedMesh').path },
