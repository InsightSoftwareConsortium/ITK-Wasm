...
class ByuReadMeshController {
  ...
  constructor(loadSampleInputs) {
    ...
    meshOutputDownload.addEventListener('click', async (event) => {
        ...
        if (model.outputs.has("mesh")) {
            const meshDownloadFormat = document.getElementById('byuReadMesh-mesh-output-format')
  ...
  async run() {
    ...
    const { couldRead, mesh, } = await meshIo.byuReadMesh(      { data: this.model.inputs.get('serializedMesh').data.slice(), path: this.model.inputs.get('serializedMesh').path },
