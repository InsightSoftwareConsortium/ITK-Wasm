...
import Mz3WritePointSetNodeOptions from './mz3-write-point-set-node-options.js'
 ...
 * @param {Mz3WritePointSetNodeOptions} options - options object
 *
 * @returns {Promise<Mz3WritePointSetNodeResult>} - result object
 */
async function mz3WritePointSetNode(
  pointSet: PointSet,
  serializedPointSet: string,
  options: Mz3WritePointSetNodeOptions = {}
) : Promise<Mz3WritePointSetNodeResult> {
  ...
  const pipelinePath = path.join(path.dirname(fileURLToPath(import.meta.url)), 'pipelines', 'mz3-write-point-set')
...
export default mz3WritePointSetNode
