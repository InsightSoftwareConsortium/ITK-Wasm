 ...
 * @param {ObjReadPointSetOptions} options - options object
 *
 * @returns {Promise<ObjReadPointSetResult>} - result object
 */
async function objReadPointSet(
  serializedPointSet: File | BinaryFile,
  options: ObjReadPointSetOptions = {}
) : Promise<ObjReadPointSetResult> {
  ...
  const pipelinePath = 'obj-read-point-set'
...
export default objReadPointSet
