 ...
 * @param {WasmZstdReadPointSetOptions} options - options object
 *
 * @returns {Promise<WasmZstdReadPointSetResult>} - result object
 */
async function wasmZstdReadPointSet(
  serializedPointSet: File | BinaryFile,
  options: WasmZstdReadPointSetOptions = {}
) : Promise<WasmZstdReadPointSetResult> {
  ...
  const pipelinePath = 'wasm-zstd-read-point-set'
...
export default wasmZstdReadPointSet
