 ...
 * @param {WasmReadPointSetOptions} options - options object
 *
 * @returns {Promise<WasmReadPointSetResult>} - result object
 */
async function wasmReadPointSet(
  serializedPointSet: File | BinaryFile,
  options: WasmReadPointSetOptions = {}
) : Promise<WasmReadPointSetResult> {
  ...
  const pipelinePath = 'wasm-read-point-set'
...
export default wasmReadPointSet
