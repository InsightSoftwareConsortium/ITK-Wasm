...
import remeshLoadSampleInputs, { usePreRun } from "./remesh-load-sample-inputs.js"
...
class RemeshController {
  ...
  constructor(loadSampleInputs) {
    ...
    this.model = new RemeshModel()
    const model = this.model

    if (loadSampleInputs) {
      const loadSampleInputsButton = document.querySelector("#remeshInputs [name=loadSampleInputs]")
    ...
    // ----------------------------------------------
    // Inputs
    const inputMeshElement = document.querySelector('#remeshInputs input[name=input-mesh-file]')
    inputMeshElement.addEventListener('change', async (event) => {
        ...
        const details = document.getElementById("remesh-input-mesh-details")
    ...
    // ----------------------------------------------
    // Options
    const numberPointsElement = document.querySelector('#remeshInputs sl-input[name=number-points]')
    numberPointsElement.addEventListener('sl-change', (event) => {
        model.options.set("numberPoints", parseFloat(numberPointsElement.value))
    })

    const triangleShapeAdaptationElement = document.querySelector('#remeshInputs sl-input[name=triangle-shape-adaptation]')
    ...
    const triangleSizeAdaptationElement = document.querySelector('#remeshInputs sl-input[name=triangle-size-adaptation]')
    ...
    const normalIterationsElement = document.querySelector('#remeshInputs sl-input[name=normal-iterations]')
    ...
    const lloydIterationsElement = document.querySelector('#remeshInputs sl-input[name=lloyd-iterations]')
    ...
    const newtonIterationsElement = document.querySelector('#remeshInputs sl-input[name=newton-iterations]')
    ...
    const newtonMElement = document.querySelector('#remeshInputs sl-input[name=newton-m]')
    newtonMElement.addEventListener('sl-change', (event) => {
        model.options.set("newtonM", parseInt(newtonMElement.value))
    })

    const lfsSamplesElement = document.querySelector('#remeshInputs sl-input[name=lfs-samples]')
    ...
    // ----------------------------------------------
    // Outputs
    const outputMeshOutputDownload = document.querySelector('#remeshOutputs sl-button[name=output-mesh-download]')
    outputMeshOutputDownload.addEventListener('click', async (event) => {
        event.preventDefault()
        event.stopPropagation()
        if (model.outputs.has("outputMesh")) {
            const outputMeshDownloadFormat = document.getElementById('remesh-output-mesh-output-format')
    ...
    const onSelectTab = async (event) => {
      if (event.detail.name === 'remesh-panel') {
        const params = new URLSearchParams(window.location.search)
        if (!params.has('functionName') || params.get('functionName') !== 'remesh') {
          params.set('functionName', 'remesh')
          const url = new URL(document.location)
          url.search = params
          window.history.replaceState({ functionName: 'remesh' }, '', url)
          await preRun()
        }
      }
    }
    ...
    function onInit() {
      const params = new URLSearchParams(window.location.search)
      if (params.has('functionName') && params.get('functionName') === 'remesh') {
        tabGroup.show('remesh-panel')
        preRun()
      }
    }
    onInit()

    const runButton = document.querySelector('#remeshInputs sl-button[name="run"]')
    runButton.addEventListener('click', async (event) => {
      ...
      try {
        ...
        globalThis.notify("remesh successfully completed", `in ${t1 - t0} milliseconds.`, "success", "rocket-fill")

        model.outputs.set("outputMesh", outputMesh)
        outputMeshOutputDownload.variant = "success"
        outputMeshOutputDownload.disabled = false
        const outputMeshDetails = document.getElementById("remesh-output-mesh-details")
  ...
  async run() {
    ...
    const { outputMesh, } = await meshFilters.remesh(      this.model.inputs.get('inputMesh'),
...
const remeshController = new RemeshController(remeshLoadSampleInputs)
