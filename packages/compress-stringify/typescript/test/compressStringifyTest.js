...
import { compressStringifyNode, parseStringDecompressNode } from '../dist/itk-compress-stringify.node.js'
...
test('We can stringify during compression', async t => {
  ...
  t.is(decoder.decode(compressedData.buffer), 'data:application/iwi+cbor+zstd;base64,KLUv/SAEIQAA3q2+7w==')
...
test('We can use a custom dataUrlPrefix', async t => {
  const data = new Uint8Array([222, 173, 190, 239])
  const { output: compressedData } = await compressStringifyNode(data, { compressionLevel: 8, stringify: true, dataUrlPrefix: 'data:application/iwm+cbor+zstd;base64,' })
  const decoder = new TextDecoder()
  t.is(decoder.decode(compressedData.buffer), 'data:application/iwm+cbor+zstd;base64,KLUv/SAEIQAA3q2+7w==')
  const { output: decompressedData } = await parseStringDecompressNode(compressedData, { parseString: true })
