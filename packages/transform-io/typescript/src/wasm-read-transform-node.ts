 ...
 * @param {WasmReadTransformNodeOptions} options - options object
 *
 * @returns {Promise<WasmReadTransformNodeResult>} - result object
 */
async function wasmReadTransformNode(
  ...
  const pipelinePath = path.join(path.dirname(fileURLToPath(import.meta.url)), 'pipelines', 'wasm-read-transform')
...
export default wasmReadTransformNode
