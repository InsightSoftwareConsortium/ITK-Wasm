 ...
 * @param {Hdf5ReadTransformNodeOptions} options - options object
 *
 * @returns {Promise<Hdf5ReadTransformNodeResult>} - result object
 */
async function hdf5ReadTransformNode(
  ...
  const pipelinePath = path.join(path.dirname(fileURLToPath(import.meta.url)), 'pipelines', 'hdf5-read-transform')
...
export default hdf5ReadTransformNode
